...
class WC_Product_Reviews_Pro_Frontend {
	...
	public function __construct() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
			...
			add_filter( 'woocommerce_structured_data_product',       array( $this, 'additional_product_structured_data' ), 10, 2 );
	...
	public function load_scripts() {
		...
		if ( ! wp_script_is( 'jquery-tiptip', 'registered' ) ) {
			wp_register_script( 'jquery-tiptip', WC()->plugin_url() . '/assets/js/jquery-tiptip/jquery.tipTip.min.js', array( 'jquery' ), ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? WC()->version : WC_VERSION ), true );
		}
	...
	public function add_author_badge( $author, $comment_id ) {
		...
		if ( $contribution && $contribution->is_type( wc_product_reviews_pro_get_enabled_contribution_types() ) ) {
			...
			$badge = wc_product_reviews_pro_author_badge( $contribution->comment );
	...
	public function customize_review_count( $count, $product ) {
		...
		if ( is_singular( 'product' ) ) {
			$count = wc_product_reviews_pro_get_comments_number( $product->get_id(), 'review' );
		}
	...
	public function improve_offer_structured_data( $offer_markup, $product ) {
		...
		$offer_markup['seller']['name'] = SV_WC_Helper::get_site_name();
	 ...
	 *
	 * @since 1.7.0
	 * @param array $markup the product schema markup
	 * @return array updated markup
	 */
	public function additional_product_structured_data( $product_markup, $product ) {
