...
class WC_Product_Reviews_Pro_Admin {
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		if ( SV_WC_Plugin_Compatibility::normalize_wc_screen_id( 'reviews' ) === $hook_suffix ) {
		...
		if (    ( SV_WC_Plugin_Compatibility::normalize_wc_screen_id() === $hook_suffix && isset( $_REQUEST['tab'] ) && 'products' == $_REQUEST['tab'] )
	...
	public function modify_contributions_query( $query ) {
		...
		if ( is_object( $screen ) && SV_WC_Plugin_Compatibility::normalize_wc_screen_id( 'reviews' ) === $screen->id ) {
	...
	public function add_meta_boxes() {
		...
		if ( isset( $_GET['c'] ) && 'comment' === get_current_screen()->id ) {
			...
			if ( in_array( $comment->comment_type, wc_product_reviews_pro()->get_contribution_types(), false ) ) {
	...
	public function filter_edit_comments_screen_translations( $translation, $text ) {
		...
		if ( in_array( $comment->comment_type, $contribution_types, false ) ) {
