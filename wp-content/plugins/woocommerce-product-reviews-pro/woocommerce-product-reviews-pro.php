...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.0', __( 'WooCommerce Product Reviews Pro', 'woocommerce-product-reviews-pro' ), __FILE__, 'init_woocommerce_product_reviews_pro', array(
...
class WC_Product_Reviews_Pro extends SV_WC_Plugin {
	...
	public function register_widgets() {
		...
		foreach ( $contribution_types as $type ) {
			...
			require_once( "includes/widgets/wc-product-reviews-pro-recent-{$type}s-widget.php" );
	...
	public function points_rewards_review_add_product_review_points( $add_points, $comment_id ) {
		...
		return in_array( $comment->comment_type, $comment_types, false );
	}
	...
	public function tab_manager_set_reviews_tab_title_review_count( $review_count, $product ) {
		...
		return wc_product_reviews_pro_get_comments_number( $product->get_id(), $enabled_contribution_types );
	}
