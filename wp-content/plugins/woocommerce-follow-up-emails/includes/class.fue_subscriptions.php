...
class FUE_Subscriptions {
    ...
    public function add_triggers( $triggers ) {
        ...
        $triggers['subs_renewal_order'] = __('after a renewal order has been created', 'follow_up_emails');
    ...
    public function add_email_triggers( $email_triggers ) {
        $email_triggers['subscription'] = array('subs_activated', 'subs_renewed', 'subs_cancelled', 'subs_expired', 'subs_suspended', 'subs_reactivated', 'subs_renewal_order', 'subs_before_renewal', 'subs_before_expire');
    ...
    public static function subscription_renewal_order_created( $renewal_order, $original_order, $product_id ) {
        ...
        self::create_email_orders($original_order->id, $triggers, $subs_key, $original_order->user_id);
    }
    ...
    public static function create_email_orders($order_id, $triggers, $subs_key = '', $user_id = '') {
        ...
        $emails = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ($trigger) AND status = ". FUE::STATUS_ACTIVE );
    ...
    public static function set_renewal_reminder( $user_id, $subs_key ) {
        ...
        $emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` = 'subs_before_renewal' AND status = ". FUE::STATUS_ACTIVE);
    ...
    public function set_expiration_reminder( $user_id, $subs_key ) {
        ...
        // look for renewal emails
        $emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` = 'subs_before_expire' AND status = ". FUE::STATUS_ACTIVE);
    ...
    public function email_variables( $vars, $email_data, $email_order, $email_row ) {
        $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{order_billing_address}', '{order_shipping_address}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{subs_renew_date}', '{subs_end_date}', '{days_to_renew}', '{item_name}', '{item_category}'));
    ...
    public function email_test_replacements( $reps ) {
        ...
        $renew_date     = date( get_option('date_format'), time()+86400);
        $end_date       = date( get_option('date_format'), time()+(86400*7) );
        $days_to_renew  = 1;
