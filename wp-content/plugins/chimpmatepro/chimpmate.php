 ...
 *
 * @wordpress-plugin
 * Plugin Name:       ChimpMate Pro - WordPress MailChimp Assistant
...
register_activation_hook( __FILE__, array( 'ChimpMatePro_WPMC_Assistant', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'ChimpMatePro_WPMC_Assistant', 'deactivate' ) );


add_action( 'plugins_loaded', array( 'ChimpMatePro_WPMC_Assistant', 'get_instance' ) );

if ( is_admin() ) {
	require_once( plugin_dir_path( __FILE__ ) . 'admin/class-chimpmate-wpmc-assistant-admin.php' );
	add_action( 'plugins_loaded', array( 'ChimpMatePro_WPMC_Assistant_Admin', 'get_instance' ) );
}
