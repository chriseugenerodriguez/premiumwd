...
#wpmchimpaf form{-webkit-backface-visibility:hidden;}
#wpmchimpaf.wpmctb_mid .wpmchimpaf{left: calc(50% - 180px);bottom: 0}
...
foreach ($form['fields'] as $f) {
  ...
  if($f['icon'] == 'idef'){
    ...
    $fi = $f['icon'];
  if($fi)
    echo '.wpmchimpaf .wpmc-ficon [wpmcfield="'.$f['tag'].'"] ~ .inputicon {background: '.$this->getIcon($fi,22,$col).' no-repeat center}';
}
?>
...
?>
  <div class="wpmchimpa-subs-button<?php echo (isset($theme['addon_button_i']) && $theme['addon_button_i'] != 'inone' && $theme['addon_button_i'] != 'idef')? ' subsicon' : '';?>"></div>
   <div class="wpmchimpa-signalc"><div class="wpmchimpa-signal"><?php
            echo $this->getSpin(isset($theme["addon_spinner_t"])?$theme["addon_spinner_t"]:'4','wpmchimpaf',isset($theme["addon_spinner_c"])?$theme["addon_spinner_c"]:'#000');?></div></div>
  </form>
