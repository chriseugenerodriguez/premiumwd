 ...
 * 
 */
class ChimpMatePro_WPMC_Assistant {
	/**
	 * @since	1.0.0
	 *
	 * @var		string
	 ...
	 * @var		string
	 */
	protected $plugin_slug = 'chimpmatepro';
	...
	private function __construct() {
		...
		add_action( 'wp_head', array( $this, 'wpmchimpa_abtest' ) );
		add_action( 'wp_head', array( $this, 'wpmchimpa_extracss' ) );
		...
		add_action('wp_ajax_nopriv_wpmchimpa_get_opt_sup', array( $this, 'wpmchimpa_get_opt' ));
		...
		add_action('wp_ajax_wpmchimpa_secret_ajax', array( $this, 'wpmchimpa_secret' ));
		add_action('wp_ajax_nopriv_wpmchimpa_secret_ajax', array( $this, 'wpmchimpa_secret' )); 

		add_filter('the_content', array( $this, 'addon_adder'));

		//woocom
		add_action('woocommerce_order_status_completed', array($this, 'woo_comp'));
		add_action('woocommerce_payment_complete', array($this, 'woo_comp'));
		add_action('woocommerce_order_status_processing', array($this, 'woo_comp'));
		add_action('woocommerce_checkout_update_order_meta', array($this, 'woo_check'));
		add_action('woocommerce_review_order_before_submit', array($this, 'woo_checkbox'));

		
		add_action( 'widgets_init', create_function( '', 'register_widget("ChimpMatePro_WPMC_Assistant_Widget");' ) );
		
	}
	/**
	 * Check if database is obsolete
	 *
	 * @since	1.0.0
	 */
	public static function wpmchimp_update_db_check(){
		$wpmchimpa=get_option('wpmchimpa_options');

		//wpmlif(!is_array($wpmchimpa)){

			$wpmchimpa=json_decode($wpmchimpa,true);
			...
			if(isset($wpmchimpa['list_record'])){
				...
				unset($wpmchimpa['list_record'],$wpmchimpa['labelnb'],$wpmchimpa['labeleb'],$wpmchimpa['namebox'],$wpmchimpa['sendwelcome']);
			}
			if(isset($wpmchimpa['topbar_field']) && isset($wpmchimpa['cforms']) && !empty($wpmchimpa['cforms'])){
				foreach ($wpmchimpa['cforms'] as $k => $v) {
					if($v['id'] == $wpmchimpa['topbar_form']){
						foreach ($v['fields'] as $y => $z) {
							if( $wpmchimpa['topbar_field'] == $z['uid']){
								$wpmchimpa['cforms'][$k]['fields'][$y]['eft'] = true;
							}
						}
						break;
					}
				}
				unset($wpmchimpa['topbar_field']);
			}
			//wpmlupdate_option('wpmchimpa_options',$wpmchimpa);
			update_option('wpmchimpa_options',json_encode($wpmchimpa));
		//wpml}
	}
	/**
	 * Update database from Basic version
	 *
	 * @since	1.0.0
	 */
	...
	public function load_plugin_textdomain() {
		...
		//wpml$this->wpmchimpa = get_option('wpmchimpa_options');
	...
	public function chimpmate_loader(){
		if(!is_single() && !is_page() && !is_404()){
			...
			$cmeta = isset( $values['chimpmate_meta'] ) ?json_decode($values['chimpmate_meta'][0],true) : array(); 
			$cat = get_the_category();
			$option_name = 'chimpmate_meta_cat_' . (isset($cat[0])?$cat[0]->term_id:'');
			$option_name_r = get_option( $option_name );
			$cmeta_cat = ($option_name_r ?json_decode($option_name_r,true) : array()); 
		}
		$this->cmeta = $cmeta;
		$this->cmeta_cat = $cmeta_cat;
		$this->isload = array();
		if(((isset($cmeta['Lightbox']['status']) && $cmeta['Lightbox']['status'] == '1') || (!isset($cmeta['Lightbox']['status']) && isset($cmeta_cat['Lightbox']['status']) && $cmeta_cat['Lightbox']['status'] == '1') || ( !isset($cmeta_cat['Lightbox']['status']) && !isset($cmeta['Lightbox']['status']) && isset($this->wpmchimpa["litebox"]) && $this->wpmchimpa_pagetype('l') && $this->wpmchimpa_exclpost('l'))) && $this->wpmchimpa_user_status() && $this->wpmchimpa_referral())
			$this->isload['l']=true;
		if(((isset($cmeta['Slider']['status']) && $cmeta['Slider']['status'] == '1') || (!isset($cmeta['Slider']['status']) && isset($cmeta_cat['Slider']['status']) && $cmeta_cat['Slider']['status'] == '1') || ( !isset($cmeta_cat['Slider']['status']) && !isset($cmeta['Slider']['status']) && isset($this->wpmchimpa["slider"]) && $this->wpmchimpa_referral() && $this->wpmchimpa_exclpost('s'))) && $this->wpmchimpa_pagetype('s') && $this->wpmchimpa_user_status())
			$this->isload['s']=true;
		if(((isset($cmeta['Addon']['status']) && $cmeta['Addon']['status'] == '1') || (!isset($cmeta['Addon']['status']) && isset($cmeta_cat['Addon']['status']) && $cmeta_cat['Addon']['status'] == '1') || ( !isset($cmeta_cat['Addon']['status']) && !isset($cmeta['Addon']['status']) && isset($this->wpmchimpa["addon"]))) && $this->wpmchimpa_user_status() && $this->wpmchimpa_referral() && $this->wpmchimpa_pagetype_addon())
			$this->isload['a']=true;
		if(((isset($cmeta['Flipbox']['status']) && $cmeta['Flipbox']['status'] == '1') || (!isset($cmeta['Flipbox']['status']) && isset($cmeta_cat['Flipbox']['status']) && $cmeta_cat['Flipbox']['status'] == '1') || ( !isset($cmeta_cat['Flipbox']['status']) && !isset($cmeta['Flipbox']['status']) && isset($this->wpmchimpa["flipbox"]) && $this->wpmchimpa_pagetype('f'))))
			$this->isload['f']=true;
		if(((isset($cmeta['Topbar']['status']) && $cmeta['Topbar']['status'] == '1') || (!isset($cmeta['Topbar']['status']) && isset($cmeta_cat['Topbar']['status']) && $cmeta_cat['Topbar']['status'] == '1') || ( !isset($cmeta_cat['Topbar']['status']) && !isset($cmeta['Topbar']['status']) && isset($this->wpmchimpa["topbar"]) && $this->wpmchimpa_pagetype('t'))))
			$this->isload['t']=true;
		if(((isset($cmeta['Widget']['status']) && $cmeta['Widget']['status'] == '1') || (!isset($cmeta['Widget']['status']) && isset($cmeta_cat['Widget']['status']) && $cmeta_cat['Widget']['status'] == '1') || ( !isset($cmeta_cat['Widget']['status']) && !isset($cmeta['Widget']['status']) && isset($this->wpmchimpa["widget"]))))
			$this->isload['w']=true;
	...
	public function enqueue_styles() {
		...
		$goo_fonts =array();
		global $mcabtheme;
		$fonts = $this->inputfont();
		$cmeta = $this->cmeta;
		$cmeta_cat = $this->cmeta_cat;
		foreach ($fonts as $font) {
			switch ($font[0]) {
				case 's':$t=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Slider']['theme'])?$cmeta['Slider']['theme'] : (isset($cmeta_cat['Slider']['theme'])?$cmeta_cat['Slider']['theme'] :$this->wpmchimpa['litebox_theme'])));
						$l = array('s'.$t);
					break;
				case 'l':$t=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Lightbox']['theme'])?$cmeta['Lightbox']['theme'] : (isset($cmeta_cat['Lightbox']['theme'])?$cmeta_cat['Lightbox']['theme'] :$this->wpmchimpa['litebox_theme'])));
						$l = array('l'.$t);
					break;
				case 'a':$t=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Addon']['theme'])?$cmeta['Addon']['theme'] : (isset($cmeta_cat['Addon']['theme'])?$cmeta_cat['Addon']['theme'] :$this->wpmchimpa['addon_theme'])));
						$u=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Flipbox']['theme'])?$cmeta['Flipbox']['theme'] : (isset($cmeta_cat['Flipbox']['theme'])?$cmeta_cat['Flipbox']['theme'] :$this->wpmchimpa['addon_theme'])));
						$v=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Topbar']['theme'])?$cmeta['Topbar']['theme'] : (isset($cmeta_cat['Topbar']['theme'])?$cmeta_cat['Topbar']['theme'] :$this->wpmchimpa['addon_theme'])));
						$l = array('a'.$t,'a'.$u,'a'.$v);
					break;
				case 'w':$t=(isset($mcabtheme)?$mcabtheme:(isset($cmeta['Widget']['theme'])?$cmeta['Widget']['theme'] : (isset($cmeta_cat['Widget']['theme'])?$cmeta_cat['Widget']['theme'] :$this->wpmchimpa['litebox_theme'])));
						$l = array('w'.$t);
					break;
			}
				
			foreach ($l as $s) {
				if (isset($this->wpmchimpa['theme'][$s][$font]) && !in_array($this->wpmchimpa['theme'][$s][$font], $this->webfont()))array_push($goo_fonts, $this->wpmchimpa['theme'][$s][$font]);
			}
	...
	public function loadscripts(){
		...
		wp_enqueue_script($this->plugin_slug . '-plugin-script');
			$opts = array();
			foreach ($this->wpmchimpa as $key => $value) {
				if(in_array($key, array("addon","addon_scode","addon_desktop","addon_tablet","addon_mobile","lite_desktop","lite_tablet","lite_mobile","lite_behave_time","lite_behave_time_inac","lite_behave_scroll","lite_behave_scrollp","lite_behave_depart","lite_behave_departs","lite_behave_onclk","lite_close_bck","topbar_scode","addon_unlock","ajax_url","suc_sub","suc_msg","slider_desktop","slider_tablet","slider_mobile","slider_orient","slider_behave_movein","slider_behave_onclk","slider_trigger_scroll","slider_trigger_scrollp","slider_trigger_scrollp;","slider_close_bck","slider_behave_depart","slider_behave_departs","slider_behave_time","slider_behave_time_inac","addon_unlock_cookie","suc_url","suc_url_tab","lite_behave_cookie","lite_behave_cookied","lite_behave_scookie","lite_close_time","lite_behave_animo","lite_behave_animodur","topbar_behave_cookie","topbar_behave_scookie","topbar_behave_cookied","flipbox_behave_cookie","flipbox_behave_scookie","flipbox_behave_scrollp","flipbox_behave_cookied","errorrf","errorfe","erroras","errorue")))
					$opts[$key] = $value;
			}
			$opts['ajax_url'] = admin_url( 'admin-ajax.php' );
			$opts['chkcon']=$this->isload;
			wp_localize_script( $this->plugin_slug . '-plugin-script', 'wpmchimpa', $opts );
	}
	/**
	 * Render the lightbox with desired settings and theme options
	 * @since	1.0.0
	 */
public function wpmchimpa_show() {
	...
	/**
	 * Render the slider with desired settings and theme options
	 * @since	1.0.0
	 */
public function wpmchimpa_slide() {	
	...
	/**
	 * Render the topbar with desired settings and theme options
	 * @since	1.0.0
	 */
public function wpmchimpa_topbar(){
	if(isset($this->isload['t'])){
		$this->loadscripts();
		include( 'includes/topbar_content.php');
	}
}
	/**
	 * Render the flipbox with desired settings and theme options
	 * @since	1.0.0
	 */
public function wpmchimpa_flipbox(){
	...
	/**
	 * Render the the social plugins if the selected theme supports
	 * @since	1.0.0
	 */
public function wpmchimpa_social(){
	if(!isset($this->social))return false;
	...
	if(isset($this->wpmchimpa['ms_api'])){
		...
		WL.init({
			...
			redirect_uri: "<?php echo WPMCA_PLUGIN_URL.'assets/ms-oauth.php';?>"
		});
		</script>
		<?php
	}
	...
	/**
	 * Return Array of fields that uses Google Fonts
	 * @since	1.0.2
	 */
public function inputfont(){
return array("lite_heading_f","lite_msg_f","lite_tbox_f","lite_check_f","lite_button_f","lite_status_f","lite_tag_f","lite_soc_f","lite_exhf","slider_heading_f","slider_msg_f","slider_check_f","slider_tbox_f","slider_button_f","slider_status_f","slider_tag_f","slider_soc_f","slider_exhf","widget_msg_f","widget_tbox_f","widget_check_f","widget_button_f","widget_status_f","widget_head_f","widget_soc_f","widget_exhf","addon_heading_f","addon_msg_f","addon_check_f","addon_tbox_f","addon_button_f","addon_status_f","addon_soc_f","addon_unlock_hf","addon_unlock_shf","addon_unlock_vf","addon_unlock_f","addon_unlock_cf","addon_exhf");
}
	/**
	 * Return list of Browser Default Fonts
	 * @since	1.0.2
	 */
public static function webfont(){
return array("Georgia, serif","Palatino Linotype, Book Antiqua, Palatino, serif","Times New Roman, Times, serif","Arial, Helvetica, sans-serif","Arial Black, Gadget, sans-serif","Comic Sans MS, cursive, sans-serif","Impact, Charcoal, sans-serif","Lucida Sans Unicode, Lucida Grande, sans-serif","Open Sans, sans-serif","Tahoma, Geneva, sans-serif","Trebuchet MS, Helvetica, sans-serif","Verdana, Geneva, sans-serif","Courier New, Courier, monospace","Lucida Console, Monaco, monospace");
	...
	/**
	 * Return true if user page matches setttings
	 * @since	1.0.0
	 *
	 */
function wpmchimpa_pagetype($e){
	...
	/**
	 * Checks the post to match the settings to Include/Exclude - boolean
	 * @since	1.0.0
	 *
	 */
function wpmchimpa_exclpost($e){
	global $wp_query;
	switch ($e) {
		case 'l':if(isset($this->wpmchimpa["lite_excl_id"])){
				$ids = explode(',', preg_replace('/\s/S', "", $this->wpmchimpa["lite_excl_id"]));
				$cid = $wp_query->post->ID;
				$res = in_array($cid, $ids);
				if(isset($this->wpmchimpa["lite_excl"])){
					if($res)return true;
					else return false;
				}else{
					if($res)return false;
			...
			break;
		case 's':if(isset($this->wpmchimpa["slider_excl_id"])){
				$ids = explode(',', preg_replace('/\s/S', "", $this->wpmchimpa["slider_excl_id"]));
				$cid = $wp_query->post->ID;
				$res = in_array($cid, $ids);
				if(isset($this->wpmchimpa["slider_excl"])){
					if($res)return true;
					else return false;
				}else{
					if($res)return false;
...
function wpmchimpa_pagetype_addon(){
	...
	/**
	 * Checks if the user status matches the settings - boolean
	 * @since	1.0.0
	 * boolean visitors login status
	 */
function wpmchimpa_user_status() {
	...
	if(is_single() && is_user_logged_in()){
		global $wp_query;
		$comm = get_comments( array('post_id' => $wp_query->post->ID,'user_id' => get_current_user_id()));
	...
	/**
	 * Checks if the referrer link matches the settings - boolean
	 * @since	1.0.0
	 * boolean visitors login status
	 */
function wpmchimpa_referral() {
	
	if(!isset($this->wpmchimpa["searchengine"]) && !isset($this->wpmchimpa["socialnet"]) && !isset($this->wpmchimpa["urlshort"]) && !isset($this->wpmchimpa["externurl"]))
		return true;
	$searchengine = array('www.google',
		'bing.com/' => array('q='),
		'search.yahoo.com',
		'ask.com/' => array('q='),
	...
	if(isset($this->wpmchimpa["searchengine"]))
		if($this->filreff($searchengine))
			return true;
	if(isset($this->wpmchimpa["socialnet"]))
		if($this->filreff($socnet))
			return true;
	if(isset($this->wpmchimpa["urlshort"]))
		if($this->filreff($ulrsh))
			return true;
	if(isset($this->wpmchimpa["externurl"]) && isset($this->wpmchimpa["externurls"])){
		$ext = explode(',', preg_replace('/\s/S', "", $this->wpmchimpa["externurls"]));
		if($this->filreff($ext))
			return true;
	}
	return false;
	...
	/**
	 * referral checking functions - boolean
	 * @since	1.0.0
	 * boolean visitors login status
	 */
function filreff($organic_sources){
	$referrer = $_SERVER["HTTP_REFERER"];
	foreach($organic_sources as $source => $queries) {
		if (is_numeric($source)) {
			if (strpos($referrer, $queries) !== false) {return true;}
		}
		else if (strpos($referrer, $source) !== false) {			
...
function get_url_cont($url){
	...
	/**
	 * Get the country code from the visitors IP - boolean
	 * @since	1.0.0
	 * boolean visitors login status
	 */

function get_weather($loc,$api) {
	if(isset($this->weat)){
		foreach ($this->weat as $value) {
			if(strpos($loc,$value->name) !== false){
				return $value;
			}
		}
	}
	...
	$this->weat = Array();
	$weat = json_decode($this->get_url_cont("http://api.openweathermap.org/data/2.5/weather?q=".$loc."&mode=json&units=metric&appid=".$api));
	if(empty($weat)) $weat = json_decode('{"coord":{"lon":-0.13,"lat":51.51},"weather":[{"id":803,"main":"Clouds","description":"broken clouds","icon":"04d"}],"base":"cmc stations","main":{"temp":0,"pressure":1020,"humidity":81,"temp_min":8,"temp_max":10},"wind":{"speed":3.1,"deg":20},"clouds":{"all":75},"dt":1444891111,"sys":{"type":1,"id":5091,"message":0.0106,"country":"GB","sunrise":1444890255,"sunset":1444928847},"id":2643743,"name":"London","cod":200}');
	$weat->cel = round($weat->main->temp);
	$weat->far = round(($weat->cel*9/5)+32);
	array_push($this->weat, $weat);
	return $weat;
}
...
<script>
jQuery(function ($) {
$('.wpmchimpa-subs-button.wpmchimpa-matbut').click(function(e){
	var target = e.target;
...
<script type="text/javascript">
jQuery(function($){
	$('.wpmcwdc').click(function(){
		p =$(this).parents('.wpmcweat');
		p.find('.wpmcwdc span').toggleClass('wpmcwdcinac');
		p.find('.wpmcwd span').toggleClass('wpmcwdinac');
	});
	$('.wpmchimpa-del-button').click(function(){
		p =$(this).parents('.wpmchimpselector').find('form');
		p[0].reset();
	});
	$('.wpmchimpa-edit-button').click(function(){
		$(this).parents('.wpmchimpselector').find('input:visible:first').focus();
	});
	var td = new Date();
	var wk = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
	var mon = ['January','February','March','April','May','June','July','August','September','November','December'];
	$('.wpmcdm').html(wk[td.getDay()]+', '+mon[td.getMonth()]);
	$('.wpmcdd').html(td.getDate());
	$('.wpmcdy').html(td.getFullYear());
});
</script>
<?php
					break;
...
<script type="text/javascript">
jQuery(function($){
	$('.wpmchimpa-min-button').click(function(){
		$('.wpmchimpa-overlay-bg').removeClass('wpmchimpa-maxwin');
		$('.wpmchimpa-overlay-bg').toggleClass('wpmchimpa-minwin');
	});
	$('.wpmchimpa-max-button').click(function(){
		$('.wpmchimpa-overlay-bg').removeClass('wpmchimpa-minwin');
		$('.wpmchimpa-overlay-bg').toggleClass('wpmchimpa-maxwin');
	});
});
</script>			
<?php
	...
	public function wpmchimpa_get_opt(){
		$opts = $this->wpmchimpa;
		unset($opts['api_key']);
	...
	public function wpmchimpa_add_email($a,$l=null){
		...
		if( $result['detail'] ){
			...
			echo 1;
			$this->wpmchimpa_abtestsubs($_POST['abtheme']);
		}
		die();
	}

	/**
	 * Render the addon and append to the content
	 * @since	1.0.0
	 * 
	 */
...
function register_shortcodes(){
	...
	if(isset($this->wpmchimpa["addon_unlock"]))
			add_shortcode('chimpmatelock', array($this,'addon_lock'));
	if(isset($this->wpmchimpa["topbar_scode"]))
			add_shortcode('chimpmate_bar', array($this,'topbar_scode'));
}
	/**
	 * Render the addon as shortcode and append to the content
	 * @since	1.0.0
	 * 
	 */
...
function topbar_scode($atts, $content = null) {
	if(function_exists(is_amp_endpoint))
		if(is_amp_endpoint())
			return false;
	...
	$this->loadscripts();
	if(isset($this->topbar_scode))$this->topbar_scode++;
	else $this->topbar_scode = 1;
	$topbar_scode = $this->topbar_scode;
	ob_start(); 
?>
<style type="text/css">
#wpmchimpat<?php echo $topbar_scode;?>{
position: relative;
height: <?=$height?>;
}
#wpmchimpat<?php echo $topbar_scode;?> .wpmchimpa-feedback.wpmchimpa-done{height: 50px;top: calc(50% - 25px);position: relative;}
#wpmchimpat<?php echo $topbar_scode;?> .wpmchimpa-feedback.wpmchimpa-done:before{float: none;}
#wpmchimpat<?php echo $topbar_scode;?> .wpmchimpat-cont{
width: calc(100% - 80px);
height: 100%;
}
#wpmchimpat<?php echo $topbar_scode;?> .wpmchimpat-close-button{
display: none;
}
#wpmchimpat<?php echo $topbar_scode;?> form{
height: 100%;
}
@media only screen and (max-width : 650px) {
#wpmchimpat<?php echo $topbar_scode;?>{height: auto;}
#wpmchimpat<?php echo $topbar_scode;?> .wpmchimpat-cont{
width: calc(100% - 40px);
}
}
...
<?php
	...
	/**
	 * Render the addon as Content Lock and append to the content
	 * @since	1.0.0
	 * 
	 */
function addon_lock($atts, $content = null) {
	if(function_exists(is_amp_endpoint))
		if(is_amp_endpoint())
			return false;
		global $mcabtheme;
		$this->loadscripts();
		if(isset($this->addonlc))$this->addonlc++;
		else $this->addonlc = 1;
		$wpmcw_id = "wpmchimpa-unlock".$this->addonlc;
$cmeta = $this->cmeta;
$cmeta_cat = $this->cmeta_cat;
		$t= (isset($mcabtheme)?$mcabtheme:(isset($cmeta['Addon']['theme'])?$cmeta['Addon']['theme'] : (isset($cmeta_cat['Addon']['theme'])?$cmeta_cat['Addon']['theme'] :$this->wpmchimpa['addon_theme'])));
		$theme = $this->wpmchimpa['theme']['a'.$t];
		global $wp_query;
		ob_start();
		?>
<script type="text/javascript">var wpmchimpa_unlock=<?=$wp_query->post->ID?>,wpmchimpa_unlocked=0;</script>
<style type="text/css">
.wpmchimpa-unlockdiv{
padding: 5px;
margin: 10px auto;
...
.wpmchimpa-unlockdiv, .wpmchimpa_lock{
background-color:#F5F5F5;
<?php
if(isset($theme["addon_unlock_bc"])){
	echo 'background-color:'.$theme["addon_unlock_bc"].';';
}
echo 'background-image:url("'.WPMCA_PLUGIN_URL.'assets/unlockpat.png");';
?>
}
.wpmchimpa_lock_h:before{
font-size: 20px;
font-weight: 600;
text-align: center;
display: block;
line-height: 40px;
<?php echo 'content:"'.(isset($theme["addon_unlock_head"])?$theme["addon_unlock_head"]:"Subscribe to Unlock").'";';
if(isset($theme["addon_unlock_hf"])){
echo 'font-family:'.$theme["addon_unlock_hf"].';';
}
if(isset($theme["addon_unlock_hfs"])){
echo 'font-size:'.$theme["addon_unlock_hfs"].'px;';
}
if(isset($theme["addon_unlock_hfw"])){
echo 'font-weight:'.$theme["addon_unlock_hfw"].';';
}
if(isset($theme["addon_unlock_hfst"])){
echo 'font-style:'.$theme["addon_unlock_hfst"].';';
}
if(isset($theme["addon_unlock_hfc"])){
echo 'color:'.$theme["addon_unlock_hfc"].';';}?>
}
.wpmchimpa_lock_h:after{
text-align: center;
display: block;
margin: -5px auto 10px;
<?php echo 'content:"'.(isset($theme["addon_unlock_shead"])?$theme["addon_unlock_shead"]:"Subscribe with your email to unlock premium content").'";';
if(isset($theme["addon_unlock_shf"])){
echo 'font-family:'.$theme["addon_unlock_shf"].';';
}
if(isset($theme["addon_unlock_shfs"])){
echo 'font-size:'.$theme["addon_unlock_shfs"].'px;';
}
if(isset($theme["addon_unlock_shfw"])){
echo 'font-weight:'.$theme["addon_unlock_shfw"].';';
}
if(isset($theme["addon_unlock_shfst"])){
echo 'font-style:'.$theme["addon_unlock_shfst"].';';
}
if(isset($theme["addon_unlock_shfc"])){
echo 'color:'.$theme["addon_unlock_shfc"].';';}?>
}
.wpmchimpa_lock_ico{
display: block;
height: 35px;
background: <?php
	$tfc='#444';
	if(isset($theme["addon_unlock_ac"])){$tfc=$theme["addon_unlock_ac"];}
	echo $this->getIcon('unlockdown',50,$tfc);?> no-repeat center;
-webkit-animation: unlocktrans 2s linear infinite;
animation: unlocktrans 2s linear infinite;
}
@-webkit-keyframes unlocktrans {
	0%	{ -webkit-transform:translateY(-15px); }
	50%	{ -webkit-transform:translateY(0); }
	100% { -webkit-transform:translateY(-15px); }
}
@keyframes unlocktrans {
	0%	{ transform:translateY(-15px); }
	50%	{ transform:translateY(0); }
	100% { transform:translateY(-15px); }
}
.wpmchimpa_lock_content{
...
.wpmchimpa_verify{padding:15px 0;text-align: center;display:none}
.wpmchimpa_verify_h:before{
font-weight: bold;
font-size: 18px;
margin-bottom: 14px;
display: block;
<?php echo 'content:"'.(isset($theme["addon_unlock_vh"])?$theme["addon_unlock_vh"]:"Confirmation message has been sent!").'";';
if(isset($theme["addon_unlock_vf"])){
echo 'font-family:'.$theme["addon_unlock_vf"].';';
}
if(isset($theme["addon_unlock_vfs"])){
echo 'font-size:'.$theme["addon_unlock_vfs"].'px;';
}
if(isset($theme["addon_unlock_vfw"])){
echo 'font-weight:'.$theme["addon_unlock_vfw"].';';
}
if(isset($theme["addon_unlock_vfst"])){
echo 'font-style:'.$theme["addon_unlock_vfst"].';';
}
if(isset($theme["addon_unlock_vfc"])){
echo 'color:'.$theme["addon_unlock_vfc"].';';
}?>
}
.wpmchimpa_verify_h:after{
<?php echo 'content:"'.(isset($theme["addon_unlock_msg"])?$theme["addon_unlock_msg"]:"We have successfully sent confirmation mail to you inbox. Kindly check your mail and verify! Please wait until verification complete").'";';
if(isset($theme["addon_unlock_f"])){
echo 'font-family:'.$theme["addon_unlock_f"].';';
}
if(isset($theme["addon_unlock_fs"])){
echo 'font-size:'.$theme["addon_unlock_fs"].'px;';
}
if(isset($theme["addon_unlock_fw"])){
echo 'font-weight:'.$theme["addon_unlock_fw"].';';
}
if(isset($theme["addon_unlock_fst"])){
echo 'font-style:'.$theme["addon_unlock_fst"].';';
}
if(isset($theme["addon_unlock_fc"])){
echo 'color:'.$theme["addon_unlock_fc"].';';
}?>
}
.wpmchimpa_verify_cb{
...
min-width: 100px;
margin-top: 15px;
<?php if(isset($theme["addon_unlock_btbc"])){
echo 'background-color:'.$theme["addon_unlock_btbc"].';';
}?>
}
.wpmchimpa_verify_cb:before{
color: #fff;
<?php echo 'content:"'.(isset($theme["addon_unlock_bt"])?$theme["addon_unlock_bt"]:"Cancel").'";';
if(isset($theme["addon_unlock_btf"])){
echo 'font-family:'.$theme["addon_unlock_btf"].';';
}
if(isset($theme["addon_unlock_btfs"])){
echo 'font-size:'.$theme["addon_unlock_btfs"].'px;';
}
if(isset($theme["addon_unlock_btfw"])){
echo 'font-weight:'.$theme["addon_unlock_btfw"].';';
}
if(isset($theme["addon_unlock_btfst"])){
echo 'font-style:'.$theme["addon_unlock_btfst"].';';
}
if(isset($theme["addon_unlock_btfc"])){
echo 'color:'.$theme["addon_unlock_btfc"].';';
}?>
}

.wpmchimpa_lock.secretload{padding-bottom: 0;margin-bottom: 40px;}
.wpmchimpa_lock.secretload .wpmchimpa_lock_content{display: none}
.wpmchimpa_lock.secretload .wpmchimpa_lock_h:after{display: none}
.wpmchimpa_lock.secretload .wpmchimpa_lock_h:before{
font-size: 20px;
font-weight: 600;
text-align: center;
display: block;
line-height: 40px;
margin-bottom: 5px;
<?php echo 'content:"'.(isset($theme["addon_unlock_vh"])?$theme["addon_unlock_vh"]:"Voila! Your Content is here...").'";';
if(isset($theme["addon_unlock_cf"])){
echo 'font-family:'.$theme["addon_unlock_cf"].';';
}
if(isset($theme["addon_unlock_cfs"])){
echo 'font-size:'.$theme["addon_unlock_cfs"].'px;';
}
if(isset($theme["addon_unlock_cfw"])){
echo 'font-weight:'.$theme["addon_unlock_cfw"].';';
}
if(isset($theme["addon_unlock_cfst"])){
echo 'font-style:'.$theme["addon_unlock_cfst"].';';
}
if(isset($theme["addon_unlock_cfc"])){
echo 'color:'.$theme["addon_unlock_cfc"].';';
}?>
}
@media only screen 
and (max-width : 320px) {
.wpmchimpa_lock_content{margin:0;}
}
</style>
<div class="wpmchimpa_lock" id="<?=$wpmcw_id?>">
	<div class="wpmchimpa_lock_h"></div>
	<div class="wpmchimpa_lock_ico"></div>
	<div class="wpmchimpa_lock_content">
		<?php include('includes/bottom_content.php');?>
		<div class="wpmchimpa_verify">
			<div class="wpmchimpa_verify_h"></div>
			<div class="wpmchimpa_verify_s">
				<?php echo $this->getSpin(isset($theme["addon_unlock_s"])?$theme["addon_unlock_s"]:'7',$wpmcw_id,isset($theme["addon_unlock_sc"])?$theme["addon_unlock_sc"]:'#000');?>
			</div>
			<div class="wpmchimpa_verify_cb"></div>
		</div>
	</div>
</div>
		<?php
$msg = ob_get_clean();
	...
	private static function single_activate() {
		...
		update_option('wpmchimpa_abtest','{"status":0}');
		update_option('wpmchimpa_prover',self::VERSION);
		if (function_exists('curl_init') && function_exists('curl_setopt')){
			$curl = curl_init();
	...
	/**
	 * Return SVG icons for the themes
	 * @since	1.0.0
	 * 
	 */
	...
	public function svglist(){
		return array(
			...
			'dd' => array(
				'path' => '<path d="M17.312,41.757h60.236c0.77,0,1.469-0.44,1.802-1.133c0.334-0.691,0.241-1.514-0.237-2.114L48.994,0.753 C48.614,0.277,48.039,0,47.431,0s-1.184,0.277-1.563,0.753L15.748,38.51c-0.479,0.601-0.572,1.423-0.238,2.114 C15.843,41.317,16.542,41.757,17.312,41.757z"/> <path d="M77.547,53.102H17.312c-0.769,0-1.469,0.439-1.802,1.133c-0.334,0.691-0.241,1.514,0.238,2.113l30.119,37.758 c0.38,0.476,0.955,0.753,1.563,0.753s1.184-0.277,1.563-0.753L79.11,56.348c0.479-0.601,0.571-1.422,0.237-2.113 C79.016,53.541,78.317,53.102,77.547,53.102z"/>',
				'dim' => 94.859
			)
		);
	}
	public function iconlist(){
		return array(
			...
			'd11' => array(
				'path' => '<path d="M200.732,116.268l-0.667-0.381L79.101,4.648c-6.801-6.198-17.862-6.198-24.663,0l-0.095,0.222 c-3.242,2.86-5.308,6.865-5.308,11.346v222.477c0,4.545,2.161,8.581,5.498,11.442l-0.064,0.064 c6.833,6.198,17.862,6.198,24.663,0l121.632-111.493C207.502,132.573,207.502,122.498,200.732,116.268z"/>',
				'dim' => 254.848
			),
			'd12' => array(
				'path' => '<path d="M200.994,4.716l0.064-0.064c-6.929-6.198-18.084-6.198-24.981-0.032L52.856,115.859 c-6.929,6.198-6.929,16.241,0,22.407l0.636,0.381L176.077,249.6c6.897,6.198,18.052,6.198,24.981,0l0.127-0.191 c3.305-2.86,5.403-6.865,5.403-11.283V16.157C206.588,11.613,204.427,7.576,200.994,4.716z"/>',
				'dim' => 254.248
			),
			'd13' => array(
			...
			'f01' => array(
				'path' => '<polygon points="83.155,234.108 102.504,306.111 130.978,306.111 166.979,194.202 137.708,194.202116.353,267.542 97.629,194.202 69.048,194.202 49.742,267.542 28.775,194.202 0,194.202 35.419,306.111 64.238,306.111"/> <polygon points="265.472,194.202 236.912,194.202 217.584,267.542 196.531,194.202 167.756,194.202203.24,306.111 232.08,306.111 251.019,234.108 270.411,306.111 298.842,306.111 334.843,194.202 305.636,194.202284.173,267.542"/> <polygon points="473.392,194.202 451.973,267.542 433.292,194.202 404.668,194.202 385.34,267.542364.395,194.202 335.577,194.202 371.082,306.111 399.879,306.111 418.818,234.108 438.146,306.111 466.662,306.111502.664,194.202"/> <path d="M251.321,35.764c-86.564,0-161.026,51.166-195.28,124.765h390.582 C412.326,86.93,337.885,35.764,251.321,35.764z"/> <path d="M251.321,466.9c86.585,0,160.961-51.144,195.28-124.765H56.084 C90.338,415.734,164.758,466.9,251.321,466.9z"/>',
				'dim' => 502.664
			),
			'f01' => array(
				'path' => '<polygon points="83.155,234.108 102.504,306.111 130.978,306.111 166.979,194.202 137.708,194.202116.353,267.542 97.629,194.202 69.048,194.202 49.742,267.542 28.775,194.202 0,194.202 35.419,306.111 64.238,306.111"/> <polygon points="265.472,194.202 236.912,194.202 217.584,267.542 196.531,194.202 167.756,194.202203.24,306.111 232.08,306.111 251.019,234.108 270.411,306.111 298.842,306.111 334.843,194.202 305.636,194.202284.173,267.542"/> <polygon points="473.392,194.202 451.973,267.542 433.292,194.202 404.668,194.202 385.34,267.542364.395,194.202 335.577,194.202 371.082,306.111 399.879,306.111 418.818,234.108 438.146,306.111 466.662,306.111502.664,194.202"/> <path d="M251.321,35.764c-86.564,0-161.026,51.166-195.28,124.765h390.582 C412.326,86.93,337.885,35.764,251.321,35.764z"/> <path d="M251.321,466.9c86.585,0,160.961-51.144,195.28-124.765H56.084 C90.338,415.734,164.758,466.9,251.321,466.9z"/>',
			...
			'j03' => array(
				'path' => '<path d="M612,116.258c-22.525,9.981-46.694,16.75-72.088,19.772c25.929-15.527,45.777-40.155,55.184-69.411 c-24.322,14.379-51.169,24.82-79.775,30.48c-22.907-24.437-55.49-39.658-91.63-39.658c-69.334,0-125.551,56.217-125.551,125.513 c0,9.828,1.109,19.427,3.251,28.606C197.065,206.32,104.556,156.337,42.641,80.386c-10.823,18.51-16.98,40.078-16.98,63.101 c0,43.559,22.181,81.993,55.835,104.479c-20.575-0.688-39.926-6.348-56.867-15.756v1.568c0,60.806,43.291,111.554,100.693,123.104 c-10.517,2.83-21.607,4.398-33.08,4.398c-8.107,0-15.947-0.803-23.634-2.333c15.985,49.907,62.336,86.199,117.253,87.194 c-42.947,33.654-97.099,53.655-155.916,53.655c-10.134,0-20.116-0.612-29.944-1.721c55.567,35.681,121.536,56.485,192.438,56.485 c230.948,0,357.188-191.291,357.188-357.188l-0.421-16.253C573.872,163.526,595.211,141.422,612,116.258z"/>',
				'dim' => 612
			),
			'j04' => array(
	...
	/**
	 * Render the extra css and append to the content
	 * @since	1.0.0
	 * 
	 */
	public function wpmchimpa_extracss(){
		echo '<style type="text/css">';
		if(isset($this->wpmchimpa['cssdesk'] ) )
			echo '@media only screen and (min-width : 1024px) {'.$this->wpmchimpa['cssdesk'].'}';
		if(isset($this->wpmchimpa['csstabl'] ) )
			echo '@media only screen and (min-width : 768px) and (max-width : 1024px) and (orientation : landscape) {'.$this->wpmchimpa['csstabl'].'}';
		if(isset($this->wpmchimpa['csstabp'] ) )
			echo '@media only screen and (min-width : 768px) and (max-width : 1024px) and (orientation : portrait) {'.$this->wpmchimpa['csstabp'].'}';
		if(isset($this->wpmchimpa['cssmobl'] ) )
			echo '@media only screen and (max-width : 750px) and (orientation : landscape) {'.$this->wpmchimpa['cssmobl'].'}';
		if(isset($this->wpmchimpa['cssmobp'] ) )
			echo '@media only screen and (max-width : 750px) and (orientation : portrait) {'.$this->wpmchimpa['cssmobp'].'}';
		echo '</style>';
	}
	/**
	 * Render the spinners for the themes
	 * @since	1.0.0
	 * 
	 */
	public function getSpin($spin,$pre,$color='#000'){
		if(!isset($spin) || !isset($pre))return '';
		switch($spin){
			case '1': return '<style type="text/css">#'.$pre.' .sp1 {border:3px solid '.$color.';border-radius:30px;height:30px;width:30px;margin: 0 auto;opacity:0;-webkit-animation: '.$pre.'sp1 1s ease-out infinite;animation: '.$pre.'sp1 1s ease-out infinite;}@-webkit-keyframes '.$pre.'sp1 {0% {-webkit-transform:scale(0.1);opacity: 0;}50% {opacity:1;}100%{-webkit-transform:scale(1.2);opacity:0;}}@keyframes '.$pre.'sp1 {0% {transform:scale(0.1);opacity: 0;}50% {opacity:1;}100% {transform:scale(1.2);opacity:0;}}</style><div class="sp1"></div>';
			case '2': return '<style type="text/css">#'.$pre.' .sp2 {margin: 0 auto;width: 30px;height: 30px;position: relative;}#'.$pre.' .sp21 > div, #'.$pre.' .sp22 > div, #'.$pre.' .sp23 > div {width: 6px;height: 6px;background-color: '.$color.';border-radius: 100%;position: absolute;-webkit-animation: '.$pre.'sp2 1.2s infinite ease-in-out;animation: '.$pre.'sp2 1.2s infinite ease-in-out;-webkit-animation-fill-mode: both;animation-fill-mode: both;}#'.$pre.' .sp2 .sp20 {position: absolute;width: 100%;height: 100%;}#'.$pre.' .sp22 {-webkit-transform: rotateZ(45deg);transform: rotateZ(45deg);}#'.$pre.' .sp23 {-webkit-transform: rotateZ(90deg);transform: rotateZ(90deg);}#'.$pre.' .sp24 { top: 0; left: 0; }#'.$pre.' .sp25 { top: 0; right: 0; }#'.$pre.' .sp26 { right: 0; bottom: 0; }#'.$pre.' .sp27 { left: 0; bottom: 0; }#'.$pre.' .sp22 .sp24 {-webkit-animation-delay: -1.1s;animation-delay: -1.1s;}#'.$pre.' .sp23 .sp24 {-webkit-animation-delay: -1.0s;animation-delay: -1.0s;}#'.$pre.' .sp21 .sp25 {-webkit-animation-delay: -0.9s;animation-delay: -0.9s;}#'.$pre.' .sp22 .sp25 {-webkit-animation-delay: -0.8s;animation-delay: -0.8s;}#'.$pre.' .sp23 .sp25 {-webkit-animation-delay: -0.7s;animation-delay: -0.7s;}#'.$pre.' .sp21 .sp26 {-webkit-animation-delay: -0.6s;animation-delay: -0.6s;}#'.$pre.' .sp22 .sp26 {-webkit-animation-delay: -0.5s;animation-delay: -0.5s;}#'.$pre.' .sp23 .sp26 {-webkit-animation-delay: -0.4s;animation-delay: -0.4s;}#'.$pre.' .sp21 .sp27 {-webkit-animation-delay: -0.3s;animation-delay: -0.3s;}#'.$pre.' .sp22 .sp27 {-webkit-animation-delay: -0.2s;animation-delay: -0.2s;}#'.$pre.' .sp23 .sp27 {-webkit-animation-delay: -0.1s;animation-delay: -0.1s;}@-webkit-keyframes '.$pre.'sp2 {0%, 80%, 100% { -webkit-transform: scale(0.0) }40% { -webkit-transform: scale(1.0) }}@keyframes '.$pre.'sp2 {0%, 80%, 100% { transform: scale(0.0);-webkit-transform: scale(0.0);} 40% { transform: scale(1.0);-webkit-transform: scale(1.0);}}</style><div class="sp2"><div class="sp20 sp21"><div class="sp24"></div><div class="sp25"></div><div class="sp26"></div><div class="sp27"></div></div><div class="sp20 sp22"><div class="sp24"></div><div class="sp25"></div><div class="sp26"></div><div class="sp27"></div></div><div class="sp20 sp23"><div class="sp24"></div><div class="sp25"></div><div class="sp26"></div><div class="sp27"></div></div></div>';
			case '3': return '<style type="text/css">#'.$pre.' .sp3 {width: 40px;height: 40px;position: relative;margin: -5px auto;}#'.$pre.' .sp3:before, #'.$pre.' .sp3:after {content: "";width: 100%;height: 100%;border-radius: 50%;background-color: '.$color.';opacity: 0.6;position: absolute;top: 0;left: 0;-webkit-animation: '.$pre.'sp3 2.0s infinite ease-in-out;animation: '.$pre.'sp3 2.0s infinite ease-in-out;}#'.$pre.' .sp3:after {-webkit-animation-delay: -1.0s;animation-delay: -1.0s;}@-webkit-keyframes '.$pre.'sp3 {0%, 100% { -webkit-transform: scale(0) }50% { -webkit-transform: scale(1) }}@keyframes '.$pre.'sp3 {0%, 100% {transform: scale(0)}50% {transform: scale(1)}}</style><div class="sp3"></div>';
			case '4': return '<style type="text/css">#'.$pre.' .sp4 {margin: 0 auto;width: 32px;height: 32px;position: relative;}#'.$pre.' .sp4:before, #'.$pre.' .sp4:before {content:"";background-color: '.$color.';width: 10px;height: 10px;position: absolute;top: 0;left: 0;-webkit-animation: '.$pre.'sp4 1.8s infinite ease-in-out;animation: '.$pre.'sp4 1.8s infinite ease-in-out;}#'.$pre.' .sp4:before {-webkit-animation-delay: -0.9s;animation-delay: -0.9s;}@-webkit-keyframes '.$pre.'sp4 {25% { -webkit-transform: translateX(21px) rotate(-90deg) scale(0.5) }50% { -webkit-transform: translateX(21px) translateY(21px) rotate(-180deg) }75% { -webkit-transform: translateX(0px) translateY(21px) rotate(-270deg) scale(0.5) }100% { -webkit-transform: rotate(-360deg) }}@keyframes '.$pre.'sp4 {25% { transform: translateX(21px) rotate(-90deg) scale(0.5);} 50% { transform: translateX(21px) translateY(21px) rotate(-179deg);} 50.1% { transform: translateX(21px) translateY(21px) rotate(-180deg);} 75% { transform: translateX(0px) translateY(21px) rotate(-270deg) scale(0.5);} 100% { transform: rotate(-360deg);}}</style><div class="sp4"></div>';
			case '5': return '<style type="text/css">#'.$pre.' .sp5 {width: 40px;height: 40px;margin: -5px auto;background-color: '.$color.';border-radius: 100%; -webkit-animation: '.$pre.'sp5 1.0s infinite ease-in-out;animation: '.$pre.'sp5 1.0s infinite ease-in-out;}@-webkit-keyframes '.$pre.'sp5 {0% { -webkit-transform: scale(0.0) }100% {-webkit-transform: scale(1.0);opacity: 0;}}@keyframes '.$pre.'sp5 {0% { transform: scale(0.0);-webkit-transform: scale(0.0);} 100% {transform: scale(1.0);-webkit-transform: scale(1.0);opacity: 0;}}</style><div class="sp5"></div>';
			case '6': return '<style type="text/css">#'.$pre.' .sp6 {margin: -7px auto;width: 40px;height: 40px;position: relative;-webkit-animation: '.$pre.'sp61 2.0s infinite linear;animation: '.$pre.'sp61 2.0s infinite linear;}#'.$pre.' .sp6:before, #'.$pre.' .sp6:after {content:"";width: 60%;height: 60%;display: inline-block;position: absolute;top: 0;background-color: '.$color.';border-radius: 100%;-webkit-animation: '.$pre.'sp60 2.0s infinite ease-in-out;animation: '.$pre.'sp60 2.0s infinite ease-in-out;}#'.$pre.' .sp6:after {top: auto;bottom: 0px;-webkit-animation-delay: -1.0s;animation-delay: -1.0s;}@-webkit-keyframes '.$pre.'sp61 { 100% { -webkit-transform: rotate(360deg) }}@keyframes '.$pre.'sp61 { 100% { transform: rotate(360deg); -webkit-transform: rotate(360deg) }}@-webkit-keyframes '.$pre.'sp60 {0%, 100% { -webkit-transform: scale(0.0) }50% { -webkit-transform: scale(1.0) }}@keyframes '.$pre.'sp60 {0%, 100% { transform: scale(0.0);-webkit-transform: scale(0.0);} 50% { transform: scale(1.0);-webkit-transform: scale(1.0);}}</style><div class="sp6"></div>';
			case '7': return '<style type="text/css">#'.$pre.' .sp7 {margin: 7px auto 0;width: 70px;}#'.$pre.' .sp7 > div {width: 18px;height: 18px;background-color: '.$color.';border-radius: 100%;display: inline-block;-webkit-animation: '.$pre.'sp7 1.4s infinite ease-in-out;animation: '.$pre.'sp7 1.4s infinite ease-in-out;-webkit-animation-fill-mode: both;animation-fill-mode: both;}#'.$pre.' .sp7 .sp71 {-webkit-animation-delay: -0.32s;animation-delay: -0.32s;}#'.$pre.' .sp7 .sp72 {-webkit-animation-delay: -0.16s;animation-delay: -0.16s;}@-webkit-keyframes '.$pre.'sp7 {0%, 80%, 100% { -webkit-transform: scale(0.0) }40% { -webkit-transform: scale(1.0) }}@keyframes '.$pre.'sp7 {0%, 80%, 100% { transform: scale(0.0);-webkit-transform: scale(0.0);} 40% { transform: scale(1.0);-webkit-transform: scale(1.0);}}</style><div class="sp7"><div class="sp71"></div><div class="sp72"></div><div class="sp73"></div></div>';
			case '8': return '<style type="text/css">#'.$pre.' .sp8 {margin: 0 auto;width: 50px;height: 30px;}#'.$pre.' .sp8 > div {background-color: '.$color.';margin-left: 3px;height: 100%;width: 6px;display: inline-block;-webkit-animation: '.$pre.'sp8 1.2s infinite ease-in-out;animation: '.$pre.'sp8 1.2s infinite ease-in-out;}#'.$pre.' .sp8 .sp82 {-webkit-animation-delay: -1.1s;animation-delay: -1.1s;}#'.$pre.' .sp8 .sp83 {-webkit-animation-delay: -1.0s;animation-delay: -1.0s;}#'.$pre.' .sp8 .sp84 {-webkit-animation-delay: -0.9s;animation-delay: -0.9s;}#'.$pre.' .sp8 .sp85 {-webkit-animation-delay: -0.8s;animation-delay: -0.8s;}@-webkit-keyframes '.$pre.'sp8 {0%, 40%, 100% { -webkit-transform: scaleY(0.4) } 20% { -webkit-transform: scaleY(1.0) }}@keyframes '.$pre.'sp8 {0%, 40%, 100% { transform: scaleY(0.4);-webkit-transform: scaleY(0.4);}  20% { transform: scaleY(1.0);-webkit-transform: scaleY(1.0);}}</style><div class="sp8"><div class="sp81"></div><div class="sp82"></div><div class="sp83"></div><div class="sp84"></div><div class="sp85"></div></div>';
		}
}
public function liteanimate(){
	if(isset($this->wpmchimpa['lite_behave_anim'])){
		$time = (isset($this->wpmchimpa['lite_behave_animdur'])?$this->wpmchimpa['lite_behave_animdur']:1);
	echo '.animated {-webkit-animation-duration: '.$time.'s; animation-duration: '.$time.'s; -webkit-animation-fill-mode: both; animation-fill-mode: both; }';
	switch ($this->wpmchimpa['lite_behave_anim']) {
		case 'bounceIn':
			?>
...
if(isset($this->wpmchimpa['lite_behave_animo'])){
		$time = (isset($this->wpmchimpa['lite_behave_animodur'])?$this->wpmchimpa['lite_behave_animodur']:1);
	echo '.animatedout {-webkit-animation-duration: '.$time.'s; animation-duration: '.$time.'s; -webkit-animation-fill-mode: both; animation-fill-mode: both; }';
	switch ($this->wpmchimpa['lite_behave_animo']) {
		case 'bounceOut':
			?>
...
.slideOutUp {-webkit-animation-name: slideOutUp; animation-name: slideOutUp; }
 ...
 <?php
	...
	/**
	 * Collect data for a/b testing - OPEN
	 * @since	1.0.0
	 * 
	 */
public function wpmchimpa_abtest(){
		global $mcabtheme;
	$abdata = json_decode(get_option('wpmchimpa_abtest'),true);
	if($abdata['status'] == 2){
		if(strtotime($abdata['end']) > time()){
			$a = array_rand($abdata['theme']);
			$mcabtheme = $abdata['theme'][$a];
			$rec['date']=date('Y-m-d');
			$rec['theme']=$mcabtheme;
			$rec['act']=0;
			if(!isset($abdata['record']))$abdata['record']=array();
			array_push($abdata['record'], $rec);
			update_option('wpmchimpa_abtest',json_encode($abdata));
		}
	}
}
	/**
	 * Collect data for a/b testing - SUBSCRIBE
	 * @since	1.0.0
	 * 
	 */
public function wpmchimpa_abtestsubs($theme){
	$abdata = json_decode(get_option('wpmchimpa_abtest'),true);
	if($abdata['status'] == 2){
		if(strtotime($abdata['end']) > time()){
			$rec['date']=date('Y-m-d');
	...
	/**
	 * Choose a theme if a/b testing is enabled
	 * @since	1.0.0
	 * 
	 */
public function wpmchimpa_abtheme(){
		global $mcabtheme;
		$abdata = json_decode(get_option('wpmchimpa_abtest'),true);
	if($abdata['status'] == 2){
		if(strtotime($abdata['end']) > time()){
			echo '<input type="hidden" name="abtheme" value="'.$mcabtheme.'">';
		}
	}
}
	/**
	 * Render the secret content for Subsribe to Unlock
	 * @since	1.0.0
	 * 
	 */
public function wpmchimpa_secret(){
	$_POST = stripslashes_deep( $_POST );
	$settings=$this->wpmchimpa;
	$api = $settings['api_key'];
	$form = $this->getformbyid($_POST['wpmcform']);
	$list = $form['list']['id'];
	$MailChimp = new ChimpMatePro_WPMC_MailChimp($api);
	$result = $MailChimp->get('/lists/'.$list.'/members/'.md5(strtolower($_POST['email'])));
	if($result["status"] == "subscribed"){
		$content_post = get_post($_POST['secret']);
		$content = $content_post->post_content;
		$r = explode('[chimpmatelock]', $content);
		if (isset($r[1])){
			$r = explode('[/chimpmatelock]', $r[1]);
			$content= $r[0];
		}
		//echo $content;
	...
	public function wpmchimpa_commentpost($comment_ID){
		if(isset($this->wpmchimpa['usyn_com']) && isset($this->wpmchimpa['usyn_comp'])){
			if(($this->wpmchimpa['usyn_comp']=='1' && isset ( $_POST['wpmchimpa'] )) || ($this->wpmchimpa['usyn_comp']=='0')){
				...
				$this->wpmchimpa_add_email(1, $this->wpmchimpa['usyn_coml']['id']);
			}
		}
	}
	...
	public function wpmchimpa_regpost(){
		if(isset($this->wpmchimpa['usyn_reg']) && isset($this->wpmchimpa['usyn_regp'])){
			...
			if(($this->wpmchimpa['usyn_regp']=='1' && isset ( $_POST['wpmchimpa'] )) || ($this->wpmchimpa['usyn_regp']=='0' && in_array($roles[0], $this->wpmchimpa['usync_role']))){
				...
				$this->wpmchimpa_add_email(1, $this->wpmchimpa['usyn_regl']['id']);
			}
		}
	}
	function getVid($t,$text) {
		...
		if(strpos($text, 'vimeo') !== FALSE) {
			$text = preg_replace('~
				...
				~ix', 
				'//player.vimeo.com/video/$1?api=1',
				$text);
				?>hide_wpmca_<?php echo $t;?>_cb = function (){
					jQuery('#wpmchimpavid<?php echo $t;?>')[0].contentWindow.postMessage('{"method":"pause"}', '*');
				}<?php
		}
		else if(strpos($text, 'dailymotion') !== FALSE) {
			$text = preg_replace('~
				 # Match Vimeo link and embed code
				https?:\/\/		# Either http or https
				(?:[\w]+\.)*		# Optional subdomains
				dailymotion\.com		# Match vimeo.com
				(?:[\/\w]*\/video?)?	# Optional video sub directory this handles groups links also
				\/			# Slash before Id
				([a-zA-Z0-9]+)		# $1: VIDEO_ID is numeric
				[^\s]*			# Not a space
				"?				# Match end quote if part of src
				~ix', 
				'//www.dailymotion.com/embed/video/$1?info=0&&api=postMessage',
				$text);
				?>hide_wpmca_<?php echo $t;?>_cb = function (){
					jQuery('#wpmchimpavid<?php echo $t;?>')[0].contentWindow.postMessage('pause', '*');
				}<?php
		}
		else{
			$text = preg_replace('~
				...
				~ix',
				'//www.youtube.com/embed/$1?controls=2&enablejsapi=1&showinfo=0',
				$text);
				?>hide_wpmca_<?php echo $t;?>_cb = function (){
					jQuery('#wpmchimpavid<?php echo $t;?>')[0].contentWindow.postMessage('{"event":"command","func":"' + 'stopVideo' + '","args":""}', '*');
				}<?php
		}
		echo "</script><iframe src='".$text."' frameborder='0' webkitAllowFullScreen mozallowfullscreen allowFullScreen id='wpmchimpavid".$t."'></iframe>";
	}
		...
		switch ($f['type']) {
			case 'email':
				if($set['type']==1)
					...
					echo '<div class="formbox wpmchimpa-field">'
						.'<div class="wpmchimpa-text'.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' wpmc-ficon':'').'">'
							.$this->fieldset($f)
							.'<div class="wpmcinfierr" wpmcerr="email"></div>'
						.'</div>'
						.'<div class="wpmchimpa-subsc"><div class="wpmchimpa-subs-button'.($set['bui'] ? ' subsicon':'').'"></div>'
						.'<div class="wpmchimpa-signal">'.$set['sig'].'</div>'
						.'</div><div style="clear:both"></div></div>';
				}
				else if ($set['type']==3){
					echo '<div class="formbox wpmchimpa-field wpmchimpa-text'
							.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' wpmc-ficon':'')
							.(($top == $k)? ' wpmctop':'')
							.(($bot == $k)? ' wpmcbot':'')
							.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?' wpmcfreq':'')
							.'">'
						.$this->fieldset($f)
						.'<div class="wpmcinfierr" wpmcerr="email"></div>'
						.'<div class="wpmchimpa-subs-button"></div>'
						.'<div class="wpmchimpa-signal">'.$set['spin'].'</div>'
						.'</div>';
				}
				else if ($set['type']==4){
					echo '<div class="wpmchimpa-field wpmchimpa-text'.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' wpmc-ficon':'').'">'
			...
			case 'imageurl':
				if($set['type']==1 || $set['type']==2){
					...
					echo '<div class="wpmchimpa-field wpmchimpa-text'
							.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' wpmc-ficon':'')
							.(($top == $k)? ' wpmctop':'')
							.(($bot == $k)? ' wpmcbot':'')
							.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?' wpmcfreq':'')
				...
				break;
			case 'checkboxes':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
...
public function gethidden($pre){
	if(!isset($pre) || empty($pre))return;
	foreach ($pre as $p) {
		$id=explode('.', $p);
		echo '<input type="hidden" name="group['.$id[1].']" value="'.$id[1].'">';
	}
}
public function woo_check($id){
	if(!class_exists( 'WooCommerce' ) || !isset($this->wpmchimpa['woocom']) || !isset($this->wpmchimpa['woocom_type']) || ($this->wpmchimpa['woocom_type'] == '1' && !isset($_POST['wpmchimpa_woocom'])))return;
		if ($this->wpmchimpa['woocom_on'] == '0')
			add_post_meta($id, 'wpmchimpa_woocom_comp', $this->wpmchimpa['woocom_type'], true);
		else if ($this->wpmchimpa['woocom_on'] == '1')
			add_post_meta($id, 'wpmchimpa_woocom_pay', $this->wpmchimpa['woocom_type'], true);
		else
			$this->woosubs($id);
}
public function woo_comp($id){
	if(!class_exists( 'WooCommerce' ) || !isset($this->wpmchimpa['woocom']) || !isset($this->wpmchimpa['woocom_type']))return;
	$woocom_comp = get_post_meta($id, 'wpmchimpa_woocom_comp', true);
	$woocom_pay = get_post_meta($id, 'wpmchimpa_woocom_pay', true);
	if(!empty($woocom_comp) || !empty($woocom_pay))
		$this->woosubs($id);

}
public function woosubs($id){
	if(!class_exists( 'WooCommerce' ))return;
	$subs = get_post_meta($id, 'wpmchimpa_woocom_subs', true);
	if(!empty($subs))return;
	$order = new WC_Order($id);
	$user = get_user_meta($order->user_id);
	$_POST['email']=$user['billing_email'][0];
	$_POST['name']=$user['billing_first_name'][0] .' '. $user['billing_last_name'][0];
	$this->wpmchimpa_add_email(1, $this->wpmchimpa['woocom_list']['id']);
	add_post_meta($id, 'wpmchimpa_woocom_subs',true,true);
	return;
}
public function woo_checkbox(){
	if(!class_exists( 'WooCommerce' ) || $this->wpmchimpa['woocom_type'] != '1')return;
	echo '<label class="wpmchimpa_woocom"><input type="checkbox" name="wpmchimpa_woocom" value="1"><span class="wpmchimpa_woocom_text" style="padding-left: 5px;">'.(isset($this->wpmchimpa['woocom_pert'])?$this->wpmchimpa['woocom_pert']:'Subscribe to Newsletters').'<span></label>';
}
