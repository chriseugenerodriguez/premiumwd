<style type="text/css">

#wpmchimpas {
width: 500px;
height: 718px;
display: block;
background-color: {{data.theme.s7.slider_canvas_c||'#333'}};
position: relative;
}
#wpmchimpas .wpmchimpas-inner{
top: 50%;
-webkit-transform: translateY(-50%);
-moz-transform: translateY(-50%);
-ms-transform: translateY(-50%);
-o-transform: translateY(-50%);
transform: translateY(-50%);
position: absolute;
margin: 0 80px;
width: 340px;
text-align: center;
-webkit-border-radius:1px;
-moz-border-radius:1px;
border-radius:1px;
background-color: {{data.theme.s7.slider_bg_c || '#f7f7f7'}}
}
#wpmchimpas .wpmchimpa-leftpane{
width:340px;
position: relative;
text-align: left;
}
#wpmchimpas .wpchimpa-banner{
height: 230px;
display: block;
position: relative;
background: #333;
background-image: url({{data.theme.s7.slider_img1 || 'data:image/svg+xml;base64,'}});
background-size: contain;
background-repeat: no-repeat;
background-position: center; 
box-shadow: 0 10px 20px rgba(0,0,0,0.19), 0 6px 6px rgba(0,0,0,0.23);
z-index: 1;
}
#wpmchimpas .wpmchimpa-leftpane:after{
	content: '';
	display: block;
	width: 55px;
	height: 55px;
	position: absolute;
	-webkit-border-radius: 50%;
	-moz-border-radius: 50%;
	border-radius: 50%;
	background: {{getIcon('b04',25,data.theme.s7.slider_hi_c||'#fff')}} no-repeat center;
	background-color: {{data.theme.s7.slider_hi_bc|| '#61c0b5'}};
	top:202px;
	right: 20px;
	z-index: 1;
box-shadow: 0 14px 28px rgba(0,0,0,0.25), 0 10px 10px rgba(0,0,0,0.22);
}
#wpmchimpas .wpmchimpa-head{
display: block;
position: absolute;
width: 100%;
bottom: 0;
}
#wpmchimpas .wpmchimpa-head:before{
content: '';
background-image: url({{data.theme.s7.slider_img1 || 'data:image/svg+xml;base64,'}});
background-position: bottom;
-webkit-filter:blur({{data.theme.s7.slider_hblur|| '15'}}px);
filter:blur({{data.theme.s7.slider_hblur|| '15'}}px);
position: absolute;
height: 100%;
width: 100%;
overflow: hidden;
left:0;
}
#wpmchimpas .wpmchimpa-headc{
  padding: 5px 100px 5px 20px;
  position: relative;
}
#wpmchimpas h3{
width: 100%;
line-height: 30px;
margin: 0;
color: {{data.theme.s7.slider_heading_fc||'#fff'}};
font-size: {{data.theme.s7.slider_heading_fs||'30'}}px;
font-weight: {{data.theme.s7.slider_heading_fw}};
font-family: {{data.theme.s7.slider_heading_f | livepf}};
font-style: {{data.theme.s7.slider_heading_fst}};
background-color: {{data.theme.s7.slider_hbg_c}};
}
#wpmchimpas .slider_msg,#wpmchimpas .slider_msg *{
font-size: 15px;
color: #fff;
font-weight: lighter;
margin: 0;
font-size: {{data.theme.s7.slider_msg_fs||'18'}}px;
font-family: {{data.theme.s7.slider_msg_f | livepf}};
}
#wpmchimpas .wpmchimpa-databox{
width: 100%;
height:100px;
display: block;
background: {{data.theme.s7.slider_exhbc || '#ededed'}};
position: relative;
}
#wpmchimpas .wpmchimpa-databox *{
	  color: {{data.theme.s7.slider_exhc1 || '#ababab'}};
	font-family: {{data.theme.s7.slider_exhf}};
	font-family: {{data.theme.s7.slider_exhfw}};
	font-family:{{data.theme.s7.slider_exhfst}}; 
}
#wpmchimpas .wpmchimpa-databox .wpmcdate{
padding: 15px 0 0 20px;
display: inline-block;
width: 160px;
position: relative;
float: left;
}
#wpmchimpas .wpmchimpa-databox .wpmcdy{
position: relative;
font-size: 25px;
line-height: 35px;
}
#wpmchimpas .wpmchimpa-databox .wpmcdm{
font-size: 16px;
line-height: 16px;
}
#wpmchimpas .wpmchimpa-databox .wpmcdd{
font-size: 65px;
line-height: 65px;
color: {{data.theme.s7.slider_exhc2 || '#f95753'}};
display: inline-block;
float: left;
}
#wpmchimpas .wpmchimpa-databox .wpmcweat{
padding: 15px 20px 10px 5px;
display: {{data.theme.s7.slider_wloc ? 'inline-block':'none'}};
width: 180px;
}
#wpmchimpas .wpmchimpa-databox .wpmcwl{
font-size: 16px;
line-height: 16px;
display: inline-block;
}
#wpmchimpas .wpmchimpa-databox .wpmcwl:after{
content: '';
width: 12px;
height: 12px;
display: inline-block;
background: {{getIcon('loc1',10,data.theme.s7.slider_exhc1||'#ababab')}} no-repeat center;
}
#wpmchimpas .wpmchimpa-databox .wpmcwc{
font-size: 15px;
line-height: 15px;
}
#wpmchimpas .wpmchimpa-databox .wpmcwi{
display: block;
width: 40px;
height: 40px;
margin: 3px 0 0 10px;
float: left;
background: {{getIcon('w01',50)}} no-repeat center;
}
#wpmchimpas .wpmchimpa-databox .wpmcwd{
position: relative;
display: inline-block;
float: left;
padding: 10px 10px 0;
font-size: 35px;
line-height: 35px;
}
#wpmchimpas .wpmchimpa-databox .wpmcwdinac{display: none;}
#wpmchimpas .wpmchimpa-databox .wpmcwdc{
display: inline-block;
font-size: 14px;
line-height: 14px;
margin: 10px 0;
}
#wpmchimpas .wpmchimpa-databox span.wpmcwdcinac{
color:#1a0dab;
cursor: pointer;
}
#wpmchimpas .wpmchimpa-databox .wpmcl2owm{
font-size: 8px;
display: {{data.theme.s7.slider_l2owm ? 'block':'none'}};
  line-height: 8px;
}
#wpmchimpas .wpmchimpa-databox .wpmcexhp{
padding: 30px 20px 0;
font-size: 15px;
}

#wpmchimpas .wpmchimpa-top{
	padding: 10px;
	text-align: center;
}
#wpmchimpas .wpmchimpa-edit-button,
#wpmchimpas .wpmchimpa-del-button{
display: block;
position: absolute;
width: 20px;
height: 20px;
top: 10px;
cursor: pointer;
opacity: 0.8;
}
#wpmchimpas .wpmchimpa-edit-button{
right: 10px;
background: {{getIcon('ed1',15,data.theme.s7.slider_ui_c||'#fff')}} no-repeat center;
}
#wpmchimpas .wpmchimpa-del-button{
right: 30px;
background: {{getIcon('del1',15,data.theme.s7.slider_ui_c||'#fff')}} no-repeat center;
}
#wpmchimpas .wpmchimpa-social{
display: inline-block;
margin-left: 10px;
}
#wpmchimpas .wpmchimpa-social::before{
content: '{{data.theme.s7.slider_soc_head || 'Subscribe with'}}';
line-height: 20px;
display: block;
float:left;
color: {{data.theme.s7.slider_soc_fc || '#fff'}};
font-size: {{data.theme.s7.slider_soc_fs||'15'}}px;
font-weight: {{data.theme.s7.slider_soc_fw}};
font-family: {{(data.theme.s7.slider_soc_f | livepf)}};
font-style: {{data.theme.s7.slider_soc_fst}};
}

#wpmchimpas .wpmchimpa-social .wpmchimpa-soc{
width:20px;
height: 20px;
margin-left:  5px;
float: left;
cursor: pointer;
-webkit-transition: all 0.1s ease;
transition: all 0.1s ease;
-webkit-backface-visibility:hidden;
border: 1px solid {{data.theme.s7.slider_soc_fc || '#fff'}};
border-radius: 50%;
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc::before{
width:18px;
height: 18px;
display: block;
content: '';
background: no-repeat center;
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-fb::before {
background-image: {{getIcon('fb1',11,data.theme.s7.slider_soc_fc||'#fff')}} 
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-gp::before {
background-image: {{getIcon('gp1',11,data.theme.s7.slider_soc_fc||'#fff')}} 
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-ms::before {
background-image: {{getIcon('ms1',11,data.theme.s7.slider_soc_fc||'#fff')}} 
}
#wpmchimpas .wpmchimpa{margin:20px;}
#wpmchimpas .slider_tbox{
text-align: left;
margin-bottom: 10px;
margin-left: 40px;
width: calc(100% - 40px);
padding: 0 20px;
-webkit-border-radius: 1px;
-moz-border-radius: 1px;
border-radius: 1px;
outline:0;
display: block;
position: relative;
color: {{data.theme.s7.slider_tbox_fc||'#61c0b5'}};
font-size: {{data.theme.s7.slider_tbox_fs||'18'}}px;
font-weight: {{data.theme.s7.slider_tbox_fw}};
font-family:Arial;
font-family: {{data.theme.s7.slider_tbox_f | livepf}};
font-style: {{data.theme.s7.slider_tbox_fst}};
background-color: {{data.theme.s7.slider_tbox_bgc || '#f7f7f7'}};
width: {{data.theme.s7.slider_tbox_w}}px;
height: {{data.theme.s7.slider_tbox_h||'40'}}px;
border-bottom: {{data.theme.s7.slider_tbox_bor||'1'}}px solid {{data.theme.s7.slider_tbox_borc||'#000'}};
}
#wpmchimpas .slider_tbox div{
top: 50%;
-webkit-transform: translatey(-50% );
-moz-transform: translatey(-50% );
-ms-transform: translatey(-50% );
-o-transform: translatey(-50% );
transform: translatey(-50% );
position: relative;
}
.slider_tbox.mailicon:before,
.slider_tbox.pericon:before{
content:'';
display: block;
width: 40px;
height:40px;
position: absolute;
top: 0;
left: -40px;
}
.slider_tbox.mailicon:before{
background: {{getIcon('a07',25,data.theme.s7.slider_inico_c||'#ababab')}} no-repeat center;
}
.slider_tbox.pericon:before{
background: {{getIcon('c05',25,data.theme.s7.slider_inico_c||'#ababab')}} no-repeat center;
}
#wpmchimpas .wpmchimpa-groups{
display: block;
margin-left: 40px;
}
#wpmchimpas .wpmchimpa-item{
display:inline-block;
margin: 2px 15px;
}
#wpmchimpas .slider_check {
cursor: pointer;
display: inline-block;
position: relative;
padding-left: 30px;
line-height: 14px;
}
#wpmchimpas .slider_check .cbox{
display: inline-block;
width: 12px;
height: 12px;
left: 0;
bottom: 0;
text-align: center;
position: absolute;
border:1px solid {{data.theme.s7.slider_check_borc||'#61c0b5'}};
background-color: {{data.theme.s7.slider_check_c}};
-webkit-border-radius: 1px;
-moz-border-radius: 1px;
border-radius: 1px;
}
#wpmchimpas .slider_check .ctext{
color: {{data.theme.s7.slider_check_fc}};
font-size: {{data.theme.s7.slider_check_fs}}px;
font-weight: {{data.theme.s7.slider_check_fw}};
font-family: {{data.theme.s7.slider_check_f | livepf}};
font-style: {{data.theme.s7.slider_check_fst}};
}
#wpmchimpas .slider_check .cbox.checked{
border:none;background:none;
}
#wpmchimpas .slider_check .cbox.checked:after{
display: block;
overflow: hidden;
width:20px;
height:20px;
content:'';
background: no-repeat center;
background-image:{{getIcon(data.theme.s7.slider_check_mark||'ch1',20,data.theme.s7.slider_check_ic||'#61c0b5')}};
}

#wpmchimpas .slider_button{
line-height: 45px;
padding-left: 10px;
margin-left: 40px;
text-align: center;
cursor: pointer;
margin-top: 15px;
position: relative;
width: calc(100% - 40px);
color: {{data.theme.s7.slider_button_fc||'#fff'}};
font-size: {{data.theme.s7.slider_button_fs || "20"}}px;
font-weight: {{data.theme.s7.slider_button_fw}};
font-family: {{data.theme.s7.slider_button_f | livepf}};
font-style: {{data.theme.s7.slider_button_fst}};
background-color:{{data.theme.s7.slider_button_bc||'#61c0b5'}};
width: {{data.theme.s7.slider_button_w}}px;
height: {{data.theme.s7.slider_button_h||'45'}}px;
-webkit-border-radius: {{data.theme.s7.slider_button_br||'1'}}px;
-moz-border-radius: {{data.theme.s7.slider_button_br||'1'}}px;
border-radius: {{data.theme.s7.slider_button_br||'1'}}px;
border: {{data.theme.s7.slider_button_bor||'1'}}px solid {{data.theme.s7.slider_button_borc||'#59c2b6'}};
}
#wpmchimpas .slider_button:hover{
color: {{data.theme.s7.slider_button_fch}};
background-color: {{data.theme.s7.slider_button_bch}};
}
.slider_spinner {
margin-top: 5px;
}

#slider_tag{
text-align: center;
margin-top: 6px;
display: {{data.theme.s7.slider_tag_en? 'block':'none'}};
}
#slider_tag,
#slider_tag *{
pointer-events: none;
color: {{data.theme.s7.slider_tag_fc||'#b8b8b8'}};
font-size: {{data.theme.s7.slider_tag_fs||'10'}}px;
font-weight: {{data.theme.s7.slider_tag_fw}};
font-family:Arial;
font-family: {{data.theme.s7.slider_tag_f | livepf}};
font-style: {{data.theme.s7.slider_tag_fst}};
}
#slider_tag:before{
content:{{getIcon('lock1',data.theme.s7.slider_tag_fs||10,data.theme.s7.slider_tag_fc||'#b8b8b8')}};
margin: 5px;
top: 1px;
position: relative;
}
#wpmchimpas-over{
background: rgba(0, 0, 0, 0.4);
height: 100%;
width: 100%;
position: absolute;
display: block;
}
#wpmchimpas-trig{
width: 50px;
height: 50px;
position: absolute;
display: block;
left: 500px;
margin: 0 3px;
box-shadow: 0 3px 6px rgba(0,0,0,0.16), 0 3px 6px rgba(0,0,0,0.23);
top:{{data.theme.s7.slider_trigger_top ||'50'}}%;
background: {{data.theme.s7.slider_trigger_bg || '#61c0b5'}};
-webkit-border-radius: 50%;
-moz-border-radius: 50%;
border-radius: 50%;
}
#wpmchimpas-trig:before{ 
content:{{getIcon('b04',28,data.theme.s7.slider_trigger_c||'#fff')}};
height: 32px;
width: 32px;
display: block;
margin:11px;
}
#wpmchimpas .wpmchimpas-inner.woexhead .wpmchimpa-databox{
display: none;
}
#wpmchimpas .wpmchimpas-inner.woexhead .wpmchimpa-leftpane{
	margin-bottom: 50px;
}
#wpmchimpas .wpmchimpas-inner.wosoc .wpmchimpa-social{
	display: none;
}
</style>
<div id="wpmchimpas-over"></div>
<div id="wpmchimpas-trig">
  <div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="6" data-lhint="Go to Trigger Options" style="top:0;right:0;margin:-10px">7</div>
</div>
<div id="wpmchimpas" class="{{data.theme.s7.slider_bg_p}}">
<div class="wpmchimpas-inner" ng-class="{'woexhead':data.theme.s7.slider_exhead,'wosoc':data.theme.s7.slider_dissoc}">
  <div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="9" data-lhint="Go to Additional Theme Options" style="margin:-15px">8</div>
        <div class="wpmchimpa-leftpane">
        	<div class="wpchimpa-banner">
	        	<div class="wpmchimpa-top">
	        		<div class="wpmchimpa-social">
		                <div class="wpmchimpa-soc wpmchimpa-fb"></div>
		                <div class="wpmchimpa-soc wpmchimpa-gp"></div>
		                <div class="wpmchimpa-soc wpmchimpa-ms"></div>
		            </div>
	        		<div class="wpmchimpa-edit-button"></div>
	        		<div class="wpmchimpa-del-button"></div>
	        	</div>
	        	<div class="wpmchimpa-head">
	        		<div class="wpmchimpa-headc">
		        		<div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="1" data-lhint="Go to Custom Message Settings">1</div>
			            <h3>{{data.theme.s7.slider_heading}}</h3>
			            <div class="slider_msg"><p ng-bind-html="data.theme.s7.slider_msg | safe"></p></div>
	        		</div>
	        	</div>
	        </div>
	        <div class="wpmchimpa-databox">
		        <div class="wpmcdate" ng-show="data.theme.s7.slider_exhopt == '0'">
		        	<div class="wpmcdm">Monday, January</div>
		        	<div class="wpmcdd">21</div>
		        	<div class="wpmcdy">2015</div>
		        </div>
		        <div class="wpmcweat" ng-show="data.theme.s7.slider_exhopt == '0'">
		        	<div class="wpmcwl">{{data.theme.s7.slider_wloc}}</div>
					<div class="wpmcwc">Clear</div>
					<div class="wpmcwi"></div>
					<div class="wpmcwd"><span>30</span><span class="wpmcwdinac">86</span></div>
					<div class="wpmcwdc"><span>&deg;C</span> |<span class="wpmcwdcinac">&deg;F</span></div>
					<div class="wpmcl2owm">extended forecast</div>
		        </div>
		        <div class="wpmcexhp" ng-show="data.theme.s7.slider_exhopt == '1'">{{data.theme.s7.slider_exhp}}</div>
	        </div>
        </div>
        <div class="wpmchimpas">
            
            <div><div class="wpmc-live-sc righthov" ng-click="gotos($event)" data-optno="2" data-lhint="Go to Text Box Settings" style="right: -20px;">2</div>
              <div class="slider_tbox pericon"><div class="in-name">Name</div></div>
              <div class="slider_tbox mailicon"><div class="in-mail">Email address</div></div>
            </div>
            <div><div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="3" data-lhint="Go to Checkbox Settings">3</div>
              <div class="wpmchimpa-groups">
                <div class="slider_check_c"></div>
                <div class="wpmchimpa-item">
                    <div class="slider_check">
                      <div class="cbox"></div>
                      <div class="ctext">group1</div>
                    </div>
                </div>
                <div class="wpmchimpa-item">
                    <div class="slider_check">
                      <div class="cbox checked"></div>
                      <div class="ctext">group2</div>
                    </div>
                </div>
              </div>
            </div>
            <div><div class="wpmc-live-sc righthov" ng-click="gotos($event)" data-optno="4" data-lhint="Go to Button Settings" style="right: -20px;">4</div>
              <div class="slider_button">{{data.theme.s7.slider_button}}</div>
            </div>
            <div><div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="8" data-lhint="Go to Tag Settings">6</div>
          <div id="slider_tag" ng-bind-html="data.theme.s7.slider_tag||'Secure and Spam free...' | safe"></div></div>
          <div>
            <div>
              <div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="5" data-lhint="Go to Spinner Settings" style="right: -20px;">5</div>
              <div class="slider_spinner" ng-bind-html="getSpin(data.theme.s7.slider_spinner_t||'1','wpmchimpas',data.theme.s7.slider_spinner_c||'#61c0b5')"></div>
            </div>
            
          </div>
           
  </div>
</div>
</div>