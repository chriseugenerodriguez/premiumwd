...
<?php
$col = ((isset($theme["widget_inico_c"]))? $theme["widget_inico_c"] : '#919191');
foreach ($form['fields'] as $f) {
  ...
  if($f['icon'] == 'idef'){
    ...
    $fi = $f['icon'];
  if($fi)
    echo '#'.$wpmcw_id.' .wpmc-ficon [wpmcfield="'.$f['tag'].'"] ~ .inputicon {background: '.$plugin->getIcon($fi,26,$col).' no-repeat center}';
}
?>
...
pointer-events: none;
  ...
  if($theme["widget_button_i"] != 'inone' && $theme["widget_button_i"] != 'idef'){
    $col = ((isset($theme["widget_button_fc"]))? $theme["widget_button_fc"] : '#fff');
     echo 'background: '.$plugin->getIcon($theme["widget_button_i"],22,$col).' no-repeat center;';
  }
  ?>
}
