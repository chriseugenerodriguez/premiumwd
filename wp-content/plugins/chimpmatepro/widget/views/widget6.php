...
background-image:<?php if(!isset($theme['widget_vid1'])){if(isset($theme['widget_img1']))echo 'url('.$theme['widget_img1'].');';else{?>
 url();<?php }} ?>
}
#<?php echo $wpmcw_id;?> .wpmchimpa-leftpane iframe, #<?php echo $wpmcw_id;?> .wpmchimpa-leftpane object, #<?php echo $wpmcw_id;?> .wpmchimpa-leftpane embed {
position: absolute;
top: 0;
left: 0;
width: 100%;
...
#<?php echo $wpmcw_id;?> .wpmc-ficon input[type="text"] ~ .inputlabel{
  ...
  <?php 
if(isset($theme["widget_tbox_h"])){
  echo 'padding-left:'.($theme["widget_tbox_h"] + 5).'px;';
  }?>
}
<?php
$col = ((isset($theme["widget_inico_c"]))? $theme["widget_inico_c"] : '#4097D3');
foreach ($form['fields'] as $f) {
  ...
  if($f['icon'] == 'idef'){
    ...
    $fi = $f['icon'];
  if($fi)
    echo '#'.$wpmcw_id.' .wpmc-ficon [wpmcfield="'.$f['tag'].'"] ~ .inputicon {background: '.$plugin->getIcon($fi,17,$col).' no-repeat center}';
}
?>
...
<?php
  ...
  if(isset($theme["widget_check_c"]))
    echo 'background: '.$theme["widget_check_c"].';';
  ?>
}
...
if(isset($theme["widget_button_bor"]) && isset($theme["widget_button_borc"])){
  $lbb=$theme["widget_button_bor"];
echo 'box-shadow:'.$lbb.'px '.$lbb.'px  0 0 solid '.$theme["widget_button_borc"].';';
}
?>
}
...
#<?php echo $wpmcw_id;?> .wpmchimpa-subs-button:active{
box-shadow:2px 2px 0 0 #FFD804;
<?php 
if(isset($theme["widget_button_bc"])){
    echo 'box-shadow:'.$lbb.'px '.$lbb.'px  0 0 solid '.$theme["widget_button_bc"].';';
}?>
}
...
pointer-events: none;
  ...
  if($theme["widget_button_i"] != 'inone' && $theme["widget_button_i"] != 'idef'){
    $col = ((isset($theme["widget_button_fc"]))? $theme["widget_button_fc"] : '#0D65A1');
     echo 'background: '.$plugin->getIcon($theme["widget_button_i"],17,$col).' no-repeat center;';
  }
  ?>
}
...
#<?php echo $wpmcw_id;?>.wotop .wpmchimpa-leftpane,
#<?php echo $wpmcw_id;?>.wosoc .wpmchimpa-social{
display: none;
}
</style>
<div class="widget-text wp_widget_plugin_box">
<div class="wpmchimpa-reset wpmchimpselector wpmchimpa chimpmatecss <?php if(isset($theme['widget_dissoc']))echo ' wosoc';
if(isset($theme['widget_disimg']))echo ' wotop';?>" id="<?php echo $wpmcw_id;?>">
         <div class="wpmchimpa-leftpane">
          <?php if(!isset($theme['widget_disimg']) && isset($theme['widget_vid1'])){
            echo $plugin->getVid('w',$theme['widget_vid1']);
          }?>
        </div>
...
?>
    ...
    echo $plugin->getSpin(isset($theme["widget_spinner_t"])?$theme["widget_spinner_t"]:'5',$wpmcw_id,isset($theme["widget_spinner_c"])?$theme["widget_spinner_c"]:'#000');?></div>
      </div>
              <?php
              if(isset($theme['widget_tag_en'])){
