...
class WC_PayPal_Standard_Subscriptions {
	...
	public static function process_paypal_ipn_request( $transaction_details ) {
		...
		switch( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				if ( ! $is_payment_change && ! $is_renewal_sign_up_after_failure && 0 == WC_Subscriptions_Order::get_total_initial_payment( $order ) ) {
				...
				if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
					...
					// Process the payment if the subscription is active
						...
						if ( true === $is_renewal_sign_up_after_failure && is_object( $renewal_order ) ) {
							...
							$renewal_order->payment_complete( $transaction_details['txn_id'] ); // WC_Subscriptions_Manager::process_subscription_payments_on_order will be called with by WC_Subscriptions_Renewal_Order::process_subscription_payment_on_child_order()
	...
	public static function paypal_standard_subscription_args( $paypal_args ) {
		...
		if ( $order_contains_failed_renewal || ( WC_Subscriptions_Order::order_contains_subscription( $order ) && WC_Subscriptions_Order::get_recurring_total( $order ) > 0 && 'yes' !== get_option( WC_Subscriptions_Admin::$option_prefix . '_turn_off_automatic_payments', 'no' ) ) ) {
			...
			$first_recurring_order_item = array_pop( $recurring_order_items );
			$product = $order->get_product_from_item( $first_recurring_order_item );
			...
			if ( $order_contains_failed_renewal ) {
				...
				$sign_up_fee               = 0;
				$initial_payment           = $renewal_order->get_total(); // Initial payment can be left in case the customer is purchased other products with the payment
	...
	public static function cancel_subscription_with_paypal( $order, $product_id = '', $profile_id = '' ) {
		...
		$response = self::change_subscription_status( $profile_id, 'Cancel', $order );
	...
	public static function suspend_subscription_with_paypal( $order, $product_id ) {
		...
		// Make sure a subscriptions status is active with PayPal
		$response = self::change_subscription_status( $profile_id, 'Suspend', $order );
	...
	public static function reactivate_subscription_with_paypal( $order, $product_id ) {
		...
		// Make sure a subscriptions status is active with PayPal
		$response = self::change_subscription_status( $profile_id, 'Reactivate', $order );
	...
	public static function change_subscription_status( $profile_id, $new_status, $order = null ) {
		...
		if ( is_wp_error( $response ) ) {
			self::$log->add( 'paypal', 'Calling PayPal to change_subscription_status for ' . $profile_id . ' has failed: ' . $response->get_error_message() . '(' . $response->get_error_code() . ')' );
		}
		...
		if( $parsed_response['ACK'] == 'Failure' ) {
			...
			if ( self::$debug ) {
				self::$log->add( 'paypal', "Calling PayPal to change_subscription_status for $profile_id has Failed: " . $parsed_response['L_LONGMESSAGE0'] );
			}
