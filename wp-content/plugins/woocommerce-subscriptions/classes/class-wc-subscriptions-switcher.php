...
class WC_Subscriptions_Switcher {
	...
	public static function add_switch_query_arg_grouped( $permalink ) {
		...
		if ( isset ( $_GET['switch-subscription'] ) && ! preg_match( '/[^0-9_]/', $_GET['switch-subscription'] ) ) {
			$permalink = add_query_arg( array( 'switch-subscription' => $_GET['switch-subscription'] ), $permalink );
		}
	...
	public static function add_switch_query_arg_post_link( $permalink, $post ) {
		...
		if ( ! isset ( $_GET['switch-subscription'] ) || preg_match( '/[^0-9_]/', $_GET['switch-subscription'] ) || ! is_main_query() || ! is_product() || 'product' !== $post->post_type ) {
			return $permalink;
		}
		...
		return add_query_arg( array( 'switch-subscription' => $_GET['switch-subscription'] ), $permalink );
	}
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( 'yes' == $apportion_sign_up_fee ) {
				...
				if ( self::order_contains_subscription_switch( $old_subscription['order_id'] ) ) {
					// Deduct any prorated amounts already paid
					$sign_up_fee_paid = $old_order->get_total();
				} else {
					// We need the product's raw sign-up fee value
					$sign_up_fee_paid = WC_Subscriptions_Order::get_item_meta( $old_order, '_subscription_sign_up_fee', $old_subscription['product_id'], 0 );
				}
			...
			if ( in_array( $apportion_recurring_price, array( 'yes', 'yes-upgrade' ) ) || ( in_array( $apportion_recurring_price, array( 'virtual', 'virtual-upgrade' ) ) && $is_virtual_product ) ) {
				...
				$old_recurring_total = WC_Subscriptions_Order::get_item_meta( $old_order, '_recurring_line_total', $old_subscription['product_id'], 0 );

				if ( 'yes' == $old_order->prices_include_tax || true === $old_order->prices_include_tax ) { // WC_Abstract_Order::$prices_include_tax can be set to true in __construct() or to 'yes' in populate()
					$old_recurring_total += WC_Subscriptions_Order::get_item_meta( $old_order, '_recurring_line_tax', $old_subscription['product_id'], 0 );
				}
	...
	public static function is_purchasable( $is_purchasable, $product ) {
		...
		if ( false === $is_purchasable && self::is_product_of_switchable_type( $product ) && WC_Subscriptions_Product::is_subscription( $product->id ) && 'no' != $product->limit_subscriptions && is_user_logged_in() && WC_Subscriptions_Manager::user_has_subscription( 0, $product->id, $product->limit_subscriptions ) ) {
