...
class WC_Subscriptions_Synchroniser {
	...
	public static function cart_contains_synced_subscription() {
		...
		if ( self::is_syncing_enabled() && isset( $woocommerce->cart ) && ! WC_Subscriptions_Cart::cart_contains_subscription_renewal( 'child' ) ) {
	...
	public static function add_settings( $settings ) {
		...
		array_splice( $settings, $index, 0, array(
			...
			array(
				...
				'type'          => 'title',
				'desc'          => sprintf( __( 'Align subscription renewal to a specific day of the week, month or year. For example, the first day of the month. %sLearn more%s.', 'woocommerce-subscriptions' ), '<a href="' . esc_url( 'http://docs.woothemes.com/document/subscriptions/renewal-synchronisation/' ) . '">', '</a>' ),
	...
	public static function is_product_synced( $product ) {
		...
		if ( ! self::is_syncing_enabled() || 'day' == $product->subscription_period || ! $product->is_type( array( 'subscription', 'variable-subscription' ) ) ) {
			return false;
		}
