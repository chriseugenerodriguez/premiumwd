...
class DP_Twitter_Widget extends WP_Widget {
	...
	function DP_Twitter_Widget() {
		...
		$this->plugin_file = 'widgets/twitter.php';
		$this->settings_url = admin_url( 'widgets.php' );
		
		$widget_ops = array('classname' => 'widget-twitter', 'description' => __( 'Display your tweets from Twitter', $this->textdomain) );
		parent::WP_Widget("{$this->prefix}-twitter-widget", __('Premiumwd Twitter Widget', $this->textdomain), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		if ( 'error' != $tweets ) {
			...
			echo '<div  id="twitter_update_list"><script type="text/javascript" src="http://twitter.com/javascripts/blogger.js"></script>
				<script type="text/javascript" src="https://twitter.com/statuses/user_timeline.json?screen_name='.$screen_name .'&include_rts=1&callback=twitterCallback2&count='.$instance['number'].'"></script></div>';
			}
		}
		echo $after_widget;
	}
