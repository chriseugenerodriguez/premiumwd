...
<div id="payment" class="woocommerce-checkout-payment">
	...
	<?php if ( $s4wc_gateway->id == 's4wc' ) : ?>
		<div class="payment_box payment_method_<?php echo $s4wc_gateway->id; ?>" <?php if ( ! $s4wc_gateway->chosen ) : ?>style="display:none;"<?php endif; ?>>
			<?php $s4wc_gateway->payment_fields(); ?>
		</div>
	<?php endif; ?>
	<?php 
	...
	<script type="text/javascript">
	    jQuery( document ).ready(function($) {		  
		   $( ".payment_methods" ).on( "click", ".input-radio", function() {
			  ...
			  if($(this).val()=='s4wc')
			  {
			  	$('.payment_box.payment_method_s4wc').slideDown();	
			  	
			  }
			  else
			  {
			  	$('.payment_box.payment_method_s4wc').slideUp();
			  }
