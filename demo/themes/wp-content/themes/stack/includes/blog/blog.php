<?php 
	...
	ob_start();
	?>
	<div id="jquery_jplayer_<?php echo $post->ID; ?>" data-id="<?php echo $post->ID; ?>" class="jp-jplayer" data-mp3="<?php echo get_post_meta( $post->ID,'_format_audio_embed',true); ?>" data-swf="<?php echo get_bloginfo( 'template_directory' ); ?>/includes/blog/swf"></div>
	...
	static $output, $args;
	ob_start(); ?>
      <div class='blog-slider'><div class='flexslider' style='margin-bottom:20px;'><ul class='slides'>
  	  <?php $attachments = get_posts( $args );
      $image_url = gallery_first_image(); if($image_url) { foreach($image_url as $i) : ?>
			<li><img width='639px' height='340px' src="<?php echo $i; ?>" /></li>
            <?php endforeach; } ?>
             </ul></div></div>
			 <?php
