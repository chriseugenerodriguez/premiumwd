  ...
  premiumwd_UI = {
    ...
    init_add: function() {
      ...
      $(document).on('click', '.option-tree-list-item-setting-add', function(e) {
        ...
        premiumwd_UI.add(this,'list_item_setting');
      });
    },
    init_edit: function() {
      $(document).on('click', '.option-tree-setting-edit', function(e) {
        ...
        if ( $(this).parents().hasClass('option-tree-setting-body') ) {
          premiumwd_UI.init_remove_active($(this),'child');
          premiumwd_UI.init_hide_body($(this),'child');
        } else {
          premiumwd_UI.init_remove_active($(this),'parent');
          premiumwd_UI.init_hide_body($(this), 'parent');
        }
        $(this).toggleClass('active');
        premiumwd_UI.init_hide_body($(this), 'toggle');
      });
    },
    init_remove: function() {
      $(document).on('click', '.option-tree-setting-remove', function(event) {
        ...
        if (agree) {
          ...
          premiumwd_UI.remove(this);
          setTimeout( function() { 
            premiumwd_UI.update_ids(list); 
          }, 200 );
        }
        return false;
      });
        ...
        $.ajax({
          ...
          complete: function( data ) {
            if ( type == 'choice' || type == 'list_item_setting' ) {
              premiumwd_UI.init_remove_active(list,'child-add');
              premiumwd_UI.init_hide_body(list,'child-add');
            } else {
              premiumwd_UI.init_remove_active();
              premiumwd_UI.init_hide_body();
            }
            ...
            setTimeout( function() {
              premiumwd_UI.init_sortable();
              premiumwd_UI.init_select_wrapper();
              premiumwd_UI.init_numeric_slider();
              premiumwd_UI.parse_condition();
            }, 500);
    ...
    update_id: function(e) {
      ...
      this.timer = setTimeout( function() {
        premiumwd_UI.update_ids($(e).parents('ul'));
      }, 100);
      return true;
    },
    ...
    condition_objects: function() {
      return 'select, input[type="radio"]:checked, input[type="text"], input[type="hidden"], input.premiumwd-numeric-slider-hidden-input';
    },
    ...
    parse_condition: function() {
      $( '.format-settings[id^="setting_"][data-condition]' ).each(function() {
        ...
        var passed;
        var conditions = premiumwd_UI.match_conditions( $( this ).data( 'condition' ) );
        var operator = ( $( this ).data( 'operator' ) || 'and' ).toLowerCase();

        $.each( conditions, function( index, condition ) {

          var target   = $( '#setting_' + condition.check );
          var targetEl = !! target.length && target.find( premiumwd_UI.condition_objects() ).first();
    ...
    init_conditions: function() {
      $('.format-settings[id^="setting_"]').on( 'change.conditionals, keyup.conditionals', premiumwd_UI.condition_objects(), function() {
        premiumwd_UI.parse_condition();
      });
      premiumwd_UI.parse_condition();
    },
    init_upload: function() {
      $(document).on('click', '.premiumwd_upload_media', function() {
        var field_id            = $(this).parent('.option-tree-ui-upload-parent').find('input').attr('id'),
            post_id             = $(this).attr('rel'),
            save_attachment_id  = $('#'+field_id).hasClass('premiumwd-upload-attachment-id'),
            btnContent          = '';
        if ( window.wp && wp.media ) {
          window.premiumwd_media_frame = window.premiumwd_media_frame || new wp.media.view.MediaFrame.Select({
          ...
          window.premiumwd_media_frame.on('select', function() {
            var attachment = window.premiumwd_media_frame.state().get('selection').first(), 
            ...
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon premiumwd-icon-minus-circle"></span>'+option_tree.remove_media_text+'</a>';
            ...
            window.premiumwd_media_frame.off('select');
          }).open();
        } else {
          ...
          window.send_to_editor = function(html) {
            ...
            if (href.match(image) && premiumwd_UI.url_exists(href)) {
              btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+href+'" alt="" /></div>';
            }
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon premiumwd-icon-minus-circle"></span>'+option_tree.remove_media_text+'</a>';
            $('#'+field_id).val(href);
            ...
            $('#'+field_id).parent().parent('div').append('<div class="option-tree-ui-media-wrap" id="'+field_id+'_media" />');
            $('#'+field_id+'_media').append(btnContent).slideDown();
            premiumwd_UI.fix_upload_parent();
            tb_remove();
    ...
    init_upload_remove: function() {
      $(document).on('click', '.option-tree-ui-remove-media', function(event) {
        ...
        var agree = confirm(option_tree.remove_agree);
        if (agree) {
          premiumwd_UI.remove_image(this);
          return false;
        }
        return false;
      });
    ...
    init_upload_fix: function(elm) {
      ...
      if ( val !== '' && ( typeof src == 'undefined' || src == false ) && premiumwd_UI.url_exists(val) ) {
        var image = /\.(?:jpe?g|png|gif|ico)$/i;
        if (val.match(image)) {
          btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+val+'" alt="" /></div>';
        }
        btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon premiumwd-icon-minus-circle">'+option_tree.remove_media_text+'</span></a>';
        $('#'+id).val(val);
        $('#'+id+'_media').remove();
        $('#'+id).parent().parent('div').append('<div class="option-tree-ui-media-wrap" id="'+id+'_media" />');
        $('#'+id+'_media').append(btnContent).slideDown();
      } else if ( val == '' || ! premiumwd_UI.url_exists(val) ) {
        $(elm).parent().next('.option-tree-ui-media-wrap').remove();
      }
    },
    init_numeric_slider: function() {
      $(".premiumwd-numeric-slider-wrap").each(function() {
        var hidden = $(".premiumwd-numeric-slider-hidden-input", this),
            value  = hidden.val(),
            helper = $(".premiumwd-numeric-slider-helper-input", this);
        if ( ! value ) {
          value = hidden.data("min");
          helper.val(value)
        }
        $(".premiumwd-numeric-slider", this).slider({
          min: hidden.data("min"),
          max: hidden.data("max"),
    ...
    fix_upload_parent: function() {
      $('.option-tree-ui-upload-input').not('.premiumwd-upload-attachment-id').on('focus blur', function(){
        $(this).parent('.option-tree-ui-upload-parent').toggleClass('focus');
        premiumwd_UI.init_upload_fix(this);
      });
    },
    ...
    css_editor_mode: function() {
      $('.premiumwd-css-editor').each(function() {
...
!function ($) {
  ...
  premiumwd_gallery = {
    ...
    frame: function (elm) {
      ...
      this._frame.on('update', function () {
        var controller = premiumwd_gallery._frame.states.get('gallery-edit')
          , library = controller.get('library')
          , ids = library.pluck('id')
          , parent = $(elm).parents('.format-setting-inner')
          , input = parent.children('.premiumwd-gallery-value')
          , shortcode = wp.media.gallery.shortcode( selection ).string().replace(/\"/g,"'")
        
        input.attr('value', ids)
                        
        if ( parent.children('.premiumwd-gallery-list').length <= 0 )
          input.after('<ul class="premiumwd-gallery-list" />')
        ...
        $.ajax({
          ...
          success: function(res) {
            parent.children('.premiumwd-gallery-list').html(res)
            if ( input.hasClass('premiumwd-gallery-shortcode') ) 
              input.val(shortcode)
            if ( $(elm).parent().children('.premiumwd-gallery-delete').length <= 0 ) {
              $(elm).parent().append('<a href="#" class="option-tree-ui-button button button-secondary hug-left premiumwd-gallery-delete">' + option_tree.delete + '</a>')
            }
  ...
  , select: function (elm) {
      var input = $(elm).parents('.format-setting-inner').children('.premiumwd-gallery-value')
        , ids = input.attr('value')
        , _shortcode = input.hasClass('premiumwd-gallery-shortcode') ? ids : '[gallery ids=\'' + ids + '\]'
        , shortcode = wp.shortcode.next('gallery', ( ids ? _shortcode : wp.media.view.settings.premiumwd_gallery.shortcode ) )
      ...
      if ( _.isUndefined( shortcode.get('ids') ) && ! input.hasClass('premiumwd-gallery-shortcode') && ids )
  ...
  , open: function (elm) {
      ...
      premiumwd_gallery.frame(elm).open()
  ...
  , remove: function (elm) {
      ...
      if ( confirm( option_tree.confirm ) ) {
        ...
        $(elm).parents('.format-setting-inner').children('.premiumwd-gallery-value').attr('value', '')
        $(elm).parents('.format-setting-inner').children('.premiumwd-gallery-list').remove()
        $(elm).next('.premiumwd-gallery-edit').text( option_tree.create )
        $(elm).remove()
        premiumwd_UI.init_conditions()
  ...
  // Gallery delete
  $(document).on('click.premiumwd_gallery.data-api', '.premiumwd-gallery-delete', function (e) {
    e.preventDefault()
    premiumwd_gallery.remove($(this))
  })
  
  // Gallery edit
  $(document).on('click.premiumwd_gallery.data-api', '.premiumwd-gallery-edit', function (e) {
    e.preventDefault()
    premiumwd_gallery.open($(this))
  })
...
!function ($) {
  ...
  $(document).on('ready', function () {
    ...
    $('.premiumwd-metabox-wrapper').each( function() {
    
      // Only if there is a tab option
      if ( $(this).find('.type-tab').length ) {
        
        // Add .premiumwd-metabox-panels
        $(this).find('.type-tab').parents('.premiumwd-metabox-wrapper').wrapInner('<div class="premiumwd-metabox-panels" />')
        
        // Wrapp with .premiumwd-metabox-tabs & add .premiumwd-metabox-nav before .premiumwd-metabox-panels
        $(this).find('.premiumwd-metabox-panels').wrap('<div class="premiumwd-metabox-tabs" />').before('<ul class="premiumwd-metabox-nav" />')
        ...
        $(this).find('.format-settings').each( function() {
          ...
          if ( $(this).find('.type-tab').length > 0 ) {
            ...
            $(this).parents('.premiumwd-metabox-panels').prev('.premiumwd-metabox-nav').append('<li><a href="#' + id + '">' + title + '</a></li>')
        ...
        // Create the tabs
        $(this).find('.premiumwd-metabox-tabs').tabs({
          activate: function( event, ui ) {
            var parent = $(this).outerHeight(),
                child = $(this).find('.premiumwd-metabox-panels').outerHeight() + 8,
                minHeight = parent - 34
            if ( $(this).find('.premiumwd-metabox-panels').css('padding') == '12px' && child < parent ) {
              $(this).find('.premiumwd-metabox-panels').css({ minHeight: minHeight })
            }
            premiumwd_UI.css_editor_mode();
          }
        })
        
        // Move the orphaned settings to the top
        $(this).find('.premiumwd-metabox-panels > .format-settings').prependTo($(this))
        
        // Remove a bunch of classes to stop style conflicts.
        $(this).find('.premiumwd-metabox-tabs').removeClass('ui-widget ui-widget-content ui-corner-all')
        $(this).find('.premiumwd-metabox-nav').removeClass('ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all')
        $(this).find('.premiumwd-metabox-nav li').removeClass('ui-state-default ui-corner-top ui-tabs-active ui-tabs-active')
        $(this).find('.premiumwd-metabox-nav li').on('hover', function() { $(this).removeClass('ui-state-hover') })
  ...
  $(document).on('ready', function () {
    ...
    $('#option-tree-settings-api .inside').each( function() {
      ...
      if ( $(this).find('.type-tab').length ) {
        ...
        // Add .premiumwd-theme-option-panels
        $(this).find('.type-tab').parents('.inside').wrapInner('<div class="premiumwd-theme-option-panels" />')
        
        // Wrap with .premiumwd-theme-option-tabs & add .premiumwd-theme-option-nav before .premiumwd-theme-option-panels
        $(this).find('.premiumwd-theme-option-panels').wrap('<div class="premiumwd-theme-option-tabs" />').before('<ul class="premiumwd-theme-option-nav" />')
        ...
        $(this).find('.format-settings').each( function() {
          ...
          if ( $(this).find('.type-tab').length > 0 ) {
            ...
            // Add a class, hide & append nav item 
            $(this).addClass('is-panel').hide()
            $(this).parents('.premiumwd-theme-option-panels').prev('.premiumwd-theme-option-nav').append('<li><a href="#' + id + '">' + title + '</a></li>')
        ...
        // Create the tabs
        $(this).find('.premiumwd-theme-option-tabs').tabs()
        
        // Move the orphaned settings to the top
        $(this).find('.premiumwd-theme-option-panels > .format-settings').prependTo($(this).find('.premiumwd-theme-option-tabs'))
      ...
      $('.wrap').on('mouseup', function () {
        ...
        $('#' + parent_id + ' input[type="radio"]').each( function () {
          ...
          if ( $(this).is(':checked') ) {
            ...
            premiumwd_checked_radios[$(this).attr('name')] = $(this).val()
          
          }
          
          // write to the object
          $(document).data('premiumwd_checked_radios', premiumwd_checked_radios)
        ...
        setTimeout( function () {
          ...
          var checked = $(document).data('premiumwd_checked_radios')
...
!function ($) {
  ...
  var formats = "input.post-format"
    , metaboxes = [
          '#premiumwd-post-format-gallery'
        , '#premiumwd-post-format-link'
        , '#premiumwd-post-format-image'
        , '#premiumwd-post-format-quote'
        , '#premiumwd-post-format-video'
        , '#premiumwd-post-format-audio'
      ]
    , ids = metaboxes.join(',')
  ...
  , init: function () {
      ...
      $( '#premiumwd-' + this.$id ).insertAfter( $( insertAfter ) ).hide()
