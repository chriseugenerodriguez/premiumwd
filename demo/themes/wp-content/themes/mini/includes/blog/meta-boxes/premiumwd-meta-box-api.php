 ...
 */
if ( ! class_exists( 'Premiumwd_Meta_Box' ) ) {

  class Premiumwd_Meta_Box {
    ...
    function add_meta_boxes() {
		...
		if(get_post_format($post_id) == $this->meta_box['id']){
    ...
    function build_meta_box( $post, $metabox ) {
      ...
      echo '<div class="premiumwd-metabox-wrapper">';
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
        ...
        if ( isset( $_POST[$field['id']] ) ) {
          ...
          if ( in_array( $field['type'], array( 'list-item', 'slider' ) ) ) {
            ...
            $settings = isset( $_POST[$field['id'] . '_settings_array'] ) ? unserialize( premiumwd_decode( $_POST[$field['id'] . '_settings_array'] ) ) : array();
            
            /* settings are empty for some odd ass reason get the defaults */
            if ( empty( $settings ) ) {
              $settings = 'slider' == $field['type'] ? 
              premiumwd_slider_settings( $field['id'] ) : 
              premiumwd_list_item_settings( $field['id'] );
            }
            ...
            foreach( $_POST[$field['id']] as $k => $setting_array ) {
              ...
              foreach( $settings as $sub_setting ) {
                ...
                if ( isset( $sub_setting['type'] ) && isset( $_POST[$field['id']][$k][$sub_setting['id']] ) ) {
                  ...
                  $_POST[$field['id']][$k][$sub_setting['id']] = premiumwd_validate_setting( $_POST[$field['id']][$k][$sub_setting['id']], $sub_setting['type'], $sub_setting['id'] );
            ...
            if ( empty( $settings ) ) {
              $settings = premiumwd_social_links_settings( $field['id'] );
            }
            ...
            $new = premiumwd_validate_setting( $_POST[$field['id']], $field['type'], $field['id'] );
          ...
          if ( $field['type'] == 'css' ) {
            ...
            if ( '' !== $new ) {
              ...
              premiumwd_insert_css_with_markers( $field['id'], $new, true );
            
            /* remove old CSS from dynamic.css */
            } else {
            
              premiumwd_remove_old_css( $field['id'] );
 ...
 * @since    2.0
 */
if ( ! function_exists( 'premiumwd_register_meta_box' ) ) {

  function premiumwd_register_meta_box( $args ) {
    if ( ! $args )
      return;
      
    $premiumwd_meta_box = new Premiumwd_Meta_Box( $args );
  }
...
/* End of file premiumwd-meta-box-api.php */
/* Location: ./includes/premiumwd-meta-box-api.php */