...
if(!defined('THEME_NAME')){
    ...
    define('THEME_MENU_TITLE', 'Mini'); //Define your menu title
    define('THEME_MENU_SLUG', 'premiumwd_theme_options'); //Define your menu slug
    define('THEME_DOMAIN', 'Mini');
    define('THEME_PRODUCT_ID', 'Mini');
...
/* Theme Deactivate */        
    ...
    public static function menuRender(){
	 ...
	 div.tooltip {  background-color:#444444;  color:White;  box-shadow:0px none;  position:absolute;  left:28px;  top:-30px;  z-index:1000000;  width:250px;  border-radius:5px;  }
	...
	<div id="premiumwd-meta-info">
	   <div id="premiumwd-meta">
		  ...
		  <ul class="theme-links">
			 ...
			 <li><a class="themes" href="http://www.premiumwd.com/wp-premium-themes/" target="_blank">More Themes</a></li>
		  </ul>
	   </div>
	</div>
	<form id="premiumwd-settings" method="post" action="">
	   <div id="premiumwd-sidebar">
		  <ul id="premiumwd-main-menu">
			 <li>
				<p><span class="fa fa-cog"></span> Activate Theme</p>
			 </li>
		  </ul>
	   </div>
	   <div id="premiumwd-content">
		  ...
		  <p>Please enter the following information to activate your theme, all information is provided upon purchase through your email and accessible on your <a target="_blank" href="https://premiumwd.com/my-account/downloads/">account</a>. If you purchased through a 3rd-party service like creative market please email us at admin@premiumwd.com if we have not contacted you already.</p>
		  <table class="form-table">
			 <tr valign="top">
				<th scope="row"><label>' . __('Email Address', THEME_DOMAIN) . ': </label></th>
				<td><input type="text" id="pw_reg_email" name="pw_reg_email" class="regular-text" value="' . $user->user_email . '"/></td>
			 </tr>
			 <tr valign="top">
				<th scope="row"><label>' . __('License Key', THEME_DOMAIN) . ': </label></th>
				<td><input type="text" id="pw_reg_key" name="pw_reg_key" class="regular-text" value=""/></td>
			 </tr>
			 <tr valign="top" class="optional">
				<th scope="row"><label>' . __('Md5 Key (Optional)', THEME_DOMAIN) . ': </label>&nbsp;<span class="question">?</span></th>
...
public static function handleForm(){
		...
		self::$params['using_domain'] = get_bloginfo('template_directory')."/includes/deactivate.php";
        ...
        if(!empty($response['activated']) && true === $response['activated']){
			...
			self::$params['key'] = self::$params['licence_key'];
			$key = new ThemeActuareVld(self::$params);
            update_option('_vld_' . THEME_DOMAIN, $key);
...
//			include TEMPLATEPATH. '/premiumwd-options/premiumwd_options.php';
    ...
    <p>' . __('Mini Theme is currently not activated', THEME_DOMAIN) . '
