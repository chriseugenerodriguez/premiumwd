	...
	jQuery(document).ready(function(){
		if(jQuery().jPlayer) {
			jQuery("#jquery-jplayer-<?php the_ID(); ?>").jPlayer({
				...
				swfPath: "<?php echo get_template_directory_uri() ?>/includes/blog/swf",
  ...
  <div class="flex-container">
    <div class="flexslider" id="flexslider-<?php the_ID(); ?>">
      <ul class="slides">
        ...
        <li>
          <?php  if (is_page_template('blog.php') || is_page_template('blog-sidebar.php') || is_single() || is_search()) {	
		  					$imageid = wp_get_attachment_image_src($image->ID,'thumb-large'); 
						} else { 
							$imageid = wp_get_attachment_image_src($image->ID,'masonry-gallery'); 
						}
						 ?>
...
<div class="post-format">
  <div class="image-container">
    <?php if ( has_post_thumbnail() ) {	
			if (is_page_template('blog.php') || is_page_template('blog-sidebar.php') || is_search()) {	the_post_thumbnail('thumb-large'); } else { the_post_thumbnail('masonry-image');}
				$caption = get_post(get_post_thumbnail_id())->post_excerpt;
				if ( isset($caption) && $caption ) echo '<div class="image-caption">'.$caption.'</div>';	
	}; ?>
  </div>
...
<div class="post-format">
  <div class="format-container group ">
   <a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"> <blockquote><?php echo isset($meta['_quote'][0])?wpautop($meta['_quote'][0]):''; ?></blockquote></a>
