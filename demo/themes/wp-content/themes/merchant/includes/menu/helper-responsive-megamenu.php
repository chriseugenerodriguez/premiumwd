...
if( !class_exists( 'premiumwd_responsive_mega_menu' ) )
{
	 ...
	 * @uses Walker
	 */
	class premiumwd_responsive_mega_menu extends Walker {
		/**
        ...
        /**
         * @var stores the data if item is hideen or shown
         */
        var $show_item = true;
		...
		*
		*/
		function premiumwd_responsive_mega_menu()
		{
			$this->grid_array = array(
...
//			if($depth === 0) $output .= "\n{replace_one}\n";
		...
		function end_lvl(&$output, $depth = 0, $args = Array() ) {
			...
			if($depth === 0)
			{
				if($this->mega_active)
				{
					...
					//$output .= "\n</div>\n";
					//$output = str_replace("{replace_one}", "<div class='premiumwd_mega_div premiumwd_mega".$this->max_columns." ".$this->grid_array[$this->max_columns]."'>", $output);
					$output = str_replace("{replace_one}", "premiumwd_mega_div ", $output);
					$output = str_replace("{last_item}", "premiumwd_mega_menu_columns_last", $output);
		...
		function start_el(&$output, $item, $depth = 0, $args = Array(), $current_object_id = 0 ) {
			...
			$this->show_item = apply_filters('premiumwd_mega_menu_hide_item', true, $output, $item, $depth, $args);
            if(empty($this->show_item)) return;
			...
			if($depth === 1 && $this->mega_active)
			{
				...
				if($this->columns > $args->max_columns || (get_post_meta( $item->ID, '_menu-item-premiumwd-division', true) && $this->columns != 1))
				{
					$this->columns = 1;
					$this->rows ++;
					$output .= "\n</ul><ul class=\"sub-menu premiumwd_mega_hr\">\n";
					$output = str_replace("{last_item}", "premiumwd_mega_menu_columns_last", $output);
				}
				else
				{
				...
				if($title != "-" && $title != '"-"') //fallback for people who copy the description o_O
				{
					...
					$item_output .= "<a href='#'><h4>".$title."</h4></a>";
				}
				...
				if($this->columns == 1)
				{
					$column_class  .= " premiumwd_mega_menu_columns_first";
				}
			}
...
if( !function_exists( 'premiumwd_responsive_fallback_menu' ) )
{
	/**
	 * Create a navigation out of pages if the user didnt create a menu in the backend
	 *
	 */
	function premiumwd_responsive_fallback_menu()
	{
		$current = "";
