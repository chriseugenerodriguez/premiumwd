...
function recent_posts_shortcode($atts)
{
	...
	while ($q->have_posts()) :
	 ...
	 ' . wp_trim_words(get_the_excerpt(), 20) . '</div></li>';
...
function my_recent_posts_shortcode($atts)
{
    ...
    mp3: "'. get_post_meta($postid,  '_format_audio_embed', true) .'",
    });
    },
...
<link rel="stylesheet" href="'. get_template_directory_uri().'/css/mosaic.css" type="text/css" media="screen" /><!--<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.6.2/jquery.min.js" type="text/javascript" charset="utf-8"></script> -->
  ...
  var total = 0;
   ...
   function mycarouselInitCallback(carousel) {
	   total = jQuery("#post-lists li.slide").length - limit ;
	  gt = total;
	  jQuery("#work-next").addClass("active");
    ...
    jQuery("#work-next").click(function() {
		if(total >= 0) total--;
		if(total <= 0) jQuery(this).removeClass("active");
		jQuery("#work-prev").addClass("active");
        carousel.next();
        return false;
    });
    ...
    jQuery("#work-prev").click(function() {
		if(total <= gt) total++;
		if(total == gt) jQuery(this).removeClass("active");
		jQuery("#work-next").addClass("active");
        carousel.prev();
        return false;
    });
...
<div id="work-prev" class="arl jcarousel-prev" data-jcarouselcontrol="true"><i class="icon-chevron-left"></i></div>
<div id="work-next" class="arr jcarousel-next" data-jcarouselcontrol="true"><i class="icon-chevron-right"></i></div>
	 </div>
	...
	while ($carouselPosts->have_posts())
	{
        ...
        <div id="jp_container_1" class="jp-audio-1">
...
<div class="gallery-slider"><ul class="slides">';
				...
				if ($image_url)
				{
					...
					foreach ($image_url as $i)
					{
						...
						$list .= '<img height="150" width="'.$width.'" src="' . $i . '" alt="' . get_the_title() . '" />';
						$list .= '</a>' . '</li>';
					}
...
function my_project_posts_shortcode($atts)
{
  ...
  function mycarousel_initCallbackp(carousel) {
	  total = jQuery("#recent-project li").length - limit ;
	  gt = total;
	  jQuery("#work-next").addClass("active");
...
<div id="work-next" class="arr jcarousel-next" data-jcarouselcontrol="true"><i class="icon-chevron-right"></i></div>
  ...
  <ul class="jcarousel-skin-tango project-item" id="recent-project">';
	...
	while ($carouselPosts->have_posts())
	{
		...
		$project .= '<li>' . '<a href="' . get_permalink() . '">' . get_the_post_thumbnail($post->ID, 'shortcode-project') . '<div class="overlay">' . '<h5>' . get_the_title() . '</h5>' . '<p>' . get_post_meta(get_the_id(), 'sub-heading', true) . '</p>' . '</div>' . '</a>' . '</li>';
	}
	
	wp_reset_query();
	
	return $project . '</ul></div>';
}
...
function shortcode_highlight($atts, $content = null)
{
	...
	return "<FONT style=\"border-radius: 2px;padding: 2px 6px;background: $color; color: $font\">$content</font>";
}
...
{
	$base =  get_template_directory_uri();
	wp_enqueue_script('alert-shortcode', $base . '/premiumwd-options/js/shortcodes.js', array ('jquery'), '1.0', true);
...
function icon_box($atts, $content = null)
{
	...
	return '<article class="feature"><div class="' . $feature . '" style="background-color:' . $color . ';"><i class="' . $type . ' ' . $icon . '"></i></div><div class="feature-description">' . $content . '</div></article>';
}
...
///////////////////////////////////////	
function pricing_sc($atts, $content = null, $code)
{
...
<div class="pricing col-<?php echo $price_count?> clearfix">
    ...
    <div class="price-table <?php if ($sub_atts[$i]['featured'] == 'true') echo 'featured'?>">
        <h2><?php echo $sub_atts[$i]['title']?></h2>
        <h4>
		<span class="price"><?php echo $sub_atts[$i]['amount']?></span>
        </h4>
        <div class="description">
		<ul>
		<?php echo $sub_contents[$i]?>
        <li class="sign-up"><a href="<?php echo $sub_atts[$i]['button_link']?>" class="join">					
		<?php echo $sub_atts[$i]['button_text']?></a></li>
        </ul>
        </div>
    </div>
...
function nivo_image_shortcode($atts, $content = null)
{
	static $slider;
	$slider = '';
	extract(shortcode_atts(array ('height' => '', 'type' => ''), $atts));
	
	if ($type=='video')
	{
		$slider .= '<li><iframe height="'.$height.'" width="100%" src="' . $content . ' ></iframe></li>';
	}
	
	else { $slider .= '<li><img height="'.$height.'" src="' . $content . '"  data-thumb="' . $content . '" /></li>';
	}
	return $slider;
}
...
function lightbox_handler($atts)
{
	...
	wp_enqueue_script('jquery-fancybox', $base . '/js/jquery.fancybox.pack.js', array ('jquery'), '1.3.4', true);
	wp_enqueue_script('fancybox-init', $base . '/js/fancybox-init.js', array ('jquery'), '1.0', true);
	wp_enqueue_style('fancybox-css', $base . '/css/jquery.fancybox.css', '1.3.4', true);
	...
	if (empty($rel))
	{
	$html .= '<a style="margin-right:20px;" class="thumb" id="' . $id . '" href="' . $url . '" title="' . $title . '"><img max-height="' . $height . '" width="' . $width . '" src="' . $url . '" alt="" />';
	}
	else
	{
		$html .= '<a style="margin-right:20px;" class="fancybox-thumb" id="' . $id . '" rel="' . $rel . '" href="' . $url . '" title="' . $title . '"><img max-height="' . $height . '" width="' . $width . '" src="' . $url . '" alt="" />';
	}
	$html .= '</a>';
...
function call_to_action($atts)
{
	...
	if ($type == '1')
	{
		$action .= '<div class="call" style="margin-bottom:40px;box-shadow: 0 2px 0 0 rgba(0, 0, 0, 0.03);background-color: #FCFCFC;
    border: 1px solid #E0E0E0;padding: 20px;border-radius:2px;">
	<div style="clear: both; overflow: hidden;"><h2 style="color: #555555; font-size: 22px; line-height: 28px;">' . $title . '</h2>
	<br/>
	<h3 style="font-size: 18px; color: #a1a1a1;">' . $subtitle . '</h3>
	...
	if ($type == '2')
	{
		$action .= '<div class="call" style="margin-bottom:20px; box-shadow: 0 2px 0 0 rgba(0, 0, 0, 0.03);background-color: #FCFCFC;
    border: 1px solid #E0E0E0;padding: 30px 30px 35px">
	<div style="width:100%"><h2 style="display:block;color: #555555; font-size: 22px; line-height: 28px;">' . $title . '</h2>
	<h3 style="display:block;margin: 20px 0px 30px;color: #888888;font-size: 14px;line-height: 22px; width:100%;">' . $subtitle . '</h3>
	<div style="display:block;"><a class="button ' . $color . ' medium" href="' . $url . '" target="' . $url . '" ><span style="color:white">' . $button . '</span></a></div>
	</div>
	</div>';
	}
	if ($type == '3')
	{
		$action .= '<div class="call" style="clear:both;padding:20px;margin-bottom:20px;height:85px;">
<h1 style="float: left;margin:0px;">' . $title . '</h1>
...
function testimonial_shortcode($atts)
{
	...
	return wpb_get_display_testimonials($title, $num);
}
...
function boxes_shortcode($atts)
{
	...
	if ($type == '1')
	{
		$boxes .= '<div class="boxes" style="box-shadow: 1px 1px 3px 0 rgba(180, 180, 180, 0.2);border: 1px solid #dedede;background-color: #fcfcfc; ">
	<div style="width:100%"><span class="text">' . $title . '</span>
	</div></div>';
	}
	if ($type == '2')
	{
		$boxes .= '<div class="boxes" style="background-color: #f8f8f8;">
	<div style="width:100%"><span class="text">' . $title . '</span>
	</div></div>';
	}
	return $boxes;
...
function teaser_shortcode($atts, $content = null)
{
	...
	if ($type == '1')
	{
		$teaser .= '<div class="teaser" style="background-color:#f7f7f7;"><img class="teaser-image" src="'.$image.'"></img><div class="content">' . do_shortcode($content) . '</div>
	</div>';
	}
	if ($type == '2')
	{
		$teaser .= '<div class="teaser" style="box-shadow: 0 2px 0 0 rgba(0, 0, 0, 0.03);"><img class="teaser-image" src="'.$image.'"></img><div class="content">' . do_shortcode($content) . '</div>
	</div>';
	}
	return $teaser;
}
...
function progress_bar_shortcode($atts, $content = null)
{
	...
	{
		$progress .= '
		<div class="bar_wrap"><div class="skill-title">'.$title.' '.$percentage.'</div>
